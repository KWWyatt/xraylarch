...
# -----------------------------------------------------------------------
# Patch FeffPathGroup so deepcopy preserves custom attributes attached by
# this pipeline (e.g. split-shell Pb-O role markers, param_type).
#
# Upstream FeffPathGroup.__getstate__/__setstate__ only round-trip a fixed
# tuple of attributes. Any setattr(path, '...', ...) we do (such as
# pb_o_split_role, pb_o_split_reff_O1/O2, param_type) is silently dropped by
# copy.deepcopy(path), which breaks iterative passes that carry forward
# stored paths via deepcopy.
# -----------------------------------------------------------------------
def _install_feffpath_state_extension():
    try:
        from larch.xafs.feffdat import FeffPathGroup
    except Exception:
        return

    if getattr(FeffPathGroup, '_xasmu2r_state_patch_installed', False):
        return
    ...
    _orig_getstate = FeffPathGroup.__getstate__
    _orig_setstate = FeffPathGroup.__setstate__

    def __getstate__(self):
        base = _orig_getstate(self)
        extras = {name: getattr(self, name) for name in _PRESERVE_ATTRS
                  if hasattr(self, name)}
        # Wrap in a tuple tagged so __setstate__ can detect the extension.
        return ('_xasmu2r_ext_v1', base, extras)

    def __setstate__(self, state):
        if (isinstance(state, tuple) and len(state) == 3
                and state[0] == '_xasmu2r_ext_v1'):
            _, base, extras = state
            _orig_setstate(self, base)
            for name, value in (extras or {}).items():
                setattr(self, name, value)
        else:
            _orig_setstate(self, state)

    FeffPathGroup.__getstate__ = __getstate__
    FeffPathGroup.__setstate__ = __setstate__
    FeffPathGroup._xasmu2r_state_patch_installed = True


_install_feffpath_state_extension()
...
class XASmu2r:
    ...
    def ft_processing_batch(self, kmax_range=(10.0, 14.0), kmin_zc_range=(1.0, 3.0),
        ...
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                ...
                zc_indices = np.where(np.diff(np.sign(weighted_chi)) != 0)[0]
                ...
                print(f"  Eligible kmax zero crossings in [{kmax_zc_lower}, {min(kmax_zc_upper, effective_kmax_upper):.2f}]: "
                      f"{ft_diagnostics['n_kmax_zero_crossings_in_range']}")
                print(f"  kmax candidates: {np.round(candidate_kmax_list, 6)}")
                if ft_diagnostics['fallback_used_for_kmax']:
                    print("  Warning: kmax fallback used because no zero crossings were found in range")
                ft_diagnostics['n_candidate_combinations_expected'] = (
                    len(candidate_kmin_list) * len(candidate_kmax_list)
                )
                print(f"  Candidate combinations to test: {ft_diagnostics['n_candidate_combinations_expected']}")
        ...
        print(f"    kmax_zc_range: {kmax_zc_range}")
    ...
    def ft_processing_batch(self, kmax_range=(10.0, 14.0), kmin_zc_range=(1.0, 3.0),
                            kweight=3, dk=3, n_kmax=10, plot_results=True,
                            first_shell_range=(1.5, 4.0), highR_range=(8.0, 12.0),
                            eps=1e-8, kmax_zc_range=None, score_tie_frac=0.05):
        """
        Perform batch Fourier Transform (FT) processing using a ratio-based score
        that favors strong first-shell signal and weak high-R leakage.

        score = first_shell_area / (highR_area + eps)
        ...
        Parameters:
            ...
            plot_results : bool
                If True, plot FT results.
            first_shell_range : tuple
                Fixed first-shell integration window in R-space.
            highR_range : tuple
                Fixed high-R leakage window in R-space.
            eps : float
                Small number to avoid divide-by-zero in score calculation.
            kmax_zc_range : tuple or None
                Optional zero-crossing range for kmax candidates. If None,
                ``kmax_range`` is used as the allowed zero-crossing range.
            score_tie_frac : float
                Relative score tolerance used to define near-tied candidates.
                Among candidates within this tolerance of the best score, prefer
                larger kmax, then larger first-shell area, then smaller high-R area.
        ...
        kmax_lower, kmax_upper = kmax_range
        kmin_zc_lower, kmin_zc_upper = kmin_zc_range
        if kmax_zc_range is None:
            kmax_zc_range = kmax_range
        kmax_zc_lower, kmax_zc_upper = kmax_zc_range
        first_shell_min, first_shell_max = first_shell_range
        highR_min, highR_max = highR_range

        def integrate_area(yvals, xvals):
            if len(xvals) < 2:
                return 0.0
            try:
                return float(integrate_method(yvals, xvals))
            except Exception:
                return float(np.trapz(yvals, xvals))
        ...
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                ...
                ft_diagnostics = {
                    ...
                    'kmax_candidates': [],
                    'n_zero_crossings_found': 0,
                    'n_kmin_zero_crossings_in_range': 0,
                    'n_kmax_zero_crossings_in_range': 0,
                    'chosen_zc_index': None,
                    'chosen_kmax_zc_index': None,
                    'kmin_at_boundary': False,
                    'kmax_at_boundary': False,
                    'kmax_boundary_side': None,
                    'selection_reason': '',
                    'fallback_used_for_kmin': False,
                    'fallback_used_for_kmax': False,
                    'first_shell_peak_amp': float('nan'),
                    'first_shell_area': float('nan'),
                    'low_r_artifact_area': float('nan'),
                    'high_r_noise_area': float('nan'),
                    'high_r_area': float('nan'),
                    'score': float('nan'),
                    'snr': float('nan'),
                    'nyquist_estimate': float('nan'),
                    'n_candidates_tested': 0,
                    'n_candidate_combinations_expected': 0,
                    'score_tie_frac': float(score_tie_frac),
                    'n_tied_candidates': 0,
                    'top_candidates': [],
                }
                ...
                candidate_kmin_list = zc_k[(zc_k >= kmin_zc_lower) & (zc_k <= kmin_zc_upper)]
                candidate_kmin_list = np.array(sorted(set(np.round(candidate_kmin_list, 6))), dtype=float)
                ft_diagnostics['n_kmin_zero_crossings_in_range'] = len(candidate_kmin_list)
                if len(candidate_kmin_list) == 0:
                    candidate_kmin_list = np.array([max(kmin_zc_lower, optimal_kmin_bkg)], dtype=float)
                    ft_diagnostics['selection_reason'] = 'fallback (no kmin zero crossings in range)'
                    ft_diagnostics['fallback_used_for_kmin'] = True

                ft_diagnostics['kmin_candidates'] = candidate_kmin_list.tolist()
                print(f"  Total weighted zero crossings found: {len(zc_k)}")
                print(f"  Eligible kmin zero crossings in [{kmin_zc_lower}, {kmin_zc_upper}]: "
                      f"{ft_diagnostics['n_kmin_zero_crossings_in_range']}")
                print(f"  kmin candidates: {np.round(candidate_kmin_list, 6)}")
                if ft_diagnostics['fallback_used_for_kmin']:
                    print("  Warning: kmin fallback used because no zero crossings were found in range")

                effective_kmax_upper = min(kmax_upper, k_max_data, optimal_kmax_bkg)
                if effective_kmax_upper < kmax_lower:
                    print(f"  Warning: No usable kmax candidates within [{kmax_lower:.2f}, {effective_kmax_upper:.2f}]")
                    continue

                candidate_kmax_list = zc_k[
                    (zc_k >= kmax_zc_lower) &
                    (zc_k <= min(kmax_zc_upper, effective_kmax_upper))
                ]
                candidate_kmax_list = np.array(sorted(set(np.round(candidate_kmax_list, 6))), dtype=float)
                ft_diagnostics['n_kmax_zero_crossings_in_range'] = len(candidate_kmax_list)
                if len(candidate_kmax_list) == 0:
                    candidate_kmax_list = np.array([effective_kmax_upper], dtype=float)
                    if ft_diagnostics['selection_reason']:
                        ft_diagnostics['selection_reason'] += '; '
                    ft_diagnostics['selection_reason'] += 'fallback (no kmax zero crossings in range)'
                    ft_diagnostics['fallback_used_for_kmax'] = True
                ft_diagnostics['kmax_candidates'] = candidate_kmax_list.tolist()

                if len(candidate_kmax_list) == 0:
                    print("  Warning: No valid kmax candidates")
                    continue

                print(f"  Eligible kmax zero crossings in [{kmax_zc_lower}, {min(kmax_zc_upper, effective_kmax_upper):.2f}]: "
                      f"{ft_diagnostics['n_kmax_zero_crossings_in_range']}")
                print(f"  kmax candidates (zero crossings): {np.round(candidate_kmax_list, 6)}")
                if ft_diagnostics['fallback_used_for_kmax']:
                    print("  Warning: kmax fallback used because no zero crossings were found in range")
                ft_diagnostics['n_candidate_combinations_expected'] = (
                    len(candidate_kmin_list) * len(candidate_kmax_list)
                ...
                for cand_kmin in candidate_kmin_list:
                    for cand_kmax in candidate_kmax_list:
                        ...
                        if not hasattr(temp, 'r') or not hasattr(temp, 'chir_mag'):
                            continue

                        r = temp.r
                        chir_mag = np.abs(temp.chir_mag)
                        first_shell_mask = (r >= first_shell_min) & (r <= first_shell_max)
                        high_r_mask = (r >= highR_min) & (r <= highR_max)
                        low_r_mask = (r > 0) & (r < 1.2)
                        noise_mask = (r > 0) & (r < rbkg_value)

                        first_shell_area = integrate_area(chir_mag[first_shell_mask], r[first_shell_mask]) if np.any(first_shell_mask) else 0.0
                        high_r_area = integrate_area(chir_mag[high_r_mask], r[high_r_mask]) if np.any(high_r_mask) else 0.0
                        low_r_area = integrate_area(chir_mag[low_r_mask], r[low_r_mask]) if np.any(low_r_mask) else 0.0
                        first_shell_peak = float(np.max(chir_mag[first_shell_mask])) if np.any(first_shell_mask) else 0.0
                        noise_level = float(np.mean(chir_mag[noise_mask])) if np.any(noise_mask) else 0.0
                        snr = first_shell_peak / max(noise_level, eps)
                        score = first_shell_area / (high_r_area + eps)

                        ft_results.append({
                            'kmin': float(cand_kmin),
                            'kmax': float(cand_kmax),
                            'score': float(score),
                            'first_shell_area': float(first_shell_area),
                            'highR_area': float(high_r_area),
                            'snr': float(snr),
                            'peak': float(first_shell_peak),
                            'lowR': float(low_r_area),
                            'noise': float(high_r_area),
                            'temp': temp,
                        })

                ft_diagnostics['n_candidates_tested'] = len(ft_results)
                if not ft_results:
                    print("  Warning: No valid FT results")
                    continue

                ranked_results = sorted(
                    ft_results,
                    key=lambda x: (-x['score'], x['highR_area'], -x['first_shell_area'], x['kmax'])
                )
                best_score = ranked_results[0]['score']
                tied_results = [
                    res for res in ranked_results
                    if res['score'] >= best_score * (1.0 - score_tie_frac)
                ]
                ft_diagnostics['n_tied_candidates'] = len(tied_results)

                best_result = sorted(
                    tied_results,
                    key=lambda x: (-x['kmax'], -x['first_shell_area'], x['highR_area'])
                )[0]
                ...
                ft_diagnostics['first_shell_peak_amp'] = best_result['peak']
                ft_diagnostics['first_shell_area'] = best_result['first_shell_area']
                ft_diagnostics['low_r_artifact_area'] = best_result['lowR']
                ft_diagnostics['high_r_noise_area'] = best_result['highR_area']
                ft_diagnostics['high_r_area'] = best_result['highR_area']
                ft_diagnostics['score'] = best_result['score']
                ft_diagnostics['snr'] = best_result['snr']

                if abs(best_result['kmin'] - np.min(candidate_kmin_list)) < 0.1:
                    ft_diagnostics['kmin_at_boundary'] = True

                if abs(best_result['kmax'] - np.min(candidate_kmax_list)) < 0.2:
                    ft_diagnostics['kmax_at_boundary'] = True
                    ft_diagnostics['kmax_boundary_side'] = 'lower'
                if abs(best_result['kmax'] - np.max(candidate_kmax_list)) < 0.2:
                    ft_diagnostics['kmax_at_boundary'] = True
                    ft_diagnostics['kmax_boundary_side'] = 'upper'

                if not ft_diagnostics['selection_reason']:
                    if len(tied_results) > 1:
                        ft_diagnostics['selection_reason'] = (
                            f'best score tie-break within {score_tie_frac:.1%} tolerance'
                        )
                    else:
                        ft_diagnostics['selection_reason'] = 'best score in range'

                chosen_idx = np.argmin(np.abs(candidate_kmin_list - best_result['kmin']))
                ft_diagnostics['chosen_zc_index'] = int(chosen_idx + 1)
                chosen_kmax_idx = np.argmin(np.abs(candidate_kmax_list - best_result['kmax']))
                ft_diagnostics['chosen_kmax_zc_index'] = int(chosen_kmax_idx + 1)
                ft_diagnostics['top_candidates'] = [
                    {
                        'kmin': res['kmin'],
                        'kmax': res['kmax'],
                        'score': res['score'],
                        'first_shell_area': res['first_shell_area'],
                        'highR_area': res['highR_area'],
                        'peak': res['peak'],
                        'lowR': res['lowR'],
                        'snr': res['snr'],
                    }
                ...
                print(f"  Best: kmin={best_result['kmin']:.2f}, kmax={best_result['kmax']:.2f}, "
                      f"score={best_result['score']:.6f}")
                print(f"        first_shell_area={best_result['first_shell_area']:.6f}, "
                      f"highR_area={best_result['highR_area']:.6f}, "
                      f"peak={best_result['peak']:.6f}, SNR={best_result['snr']:.3f}")
                print(f"        selected zero-crossing indices: kmin #{ft_diagnostics['chosen_zc_index']}, "
                      f"kmax #{ft_diagnostics['chosen_kmax_zc_index']}")
                if ft_diagnostics['kmax_at_boundary']:
                    print(f"  Warning: selected kmax is at the {ft_diagnostics['kmax_boundary_side']} search bound")
                ...
                if plot_results:
                    ...
                    ax1.plot(best_result['temp'].r, best_result['temp'].chir_re,
                             'r--', lw=1.5, label='Re[χ(R)]')
                    ax1.axvspan(0, rbkg_value, color='lightgray', alpha=0.3, label='Low-R noise')
                    ax1.axvspan(first_shell_min, first_shell_max, color='green', alpha=0.15, label='First-shell window')
                    ax1.axvspan(highR_min, highR_max, color='orange', alpha=0.12, label='High-R window')
                    ax1.set_xlabel('R (Å)', fontsize=12)
                    ax1.set_ylabel('|χ(R)|', fontsize=12)
                    ax1.set_title(
                        f'Best FT: kmin={best_result["kmin"]:.2f}, kmax={best_result["kmax"]:.2f}\n'
                        f'score={best_result["score"]:.4f}'
                    )
                    ax1.legend()
                    ax1.set_xlim(0, max(6.0, highR_max))
                    ax1.grid(True, alpha=0.3)
        ...
        print(f"    kweight: {kweight}")
        print(f"    dk: {dk}")
        print(f"    first_shell_range: {first_shell_range}")
        print(f"    highR_range: {highR_range}")
        print(f"    eps: {eps}")
        print(f"    score_tie_frac: {score_tie_frac}")
        ...
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                ...
                diag = getattr(group, 'ft_diagnostics', {})
                print(f"\n  {proj_name}.{group_name}:")
                print(f"    Selected: kmin={group.best_kmin:.2f}, kmax={group.best_kmax:.2f}")
                print(f"    Score: {diag.get('score', float('nan')):.6f}")
                print(f"    First-shell area: {diag.get('first_shell_area', float('nan')):.6f}")
                print(f"    High-R area: {diag.get('high_r_area', float('nan')):.6f}")
                print(f"    First-shell peak height: {diag.get('first_shell_peak_amp', float('nan')):.6f}")
                print(f"    Low-R artifact area (0-1.2 Å): {diag.get('low_r_artifact_area', float('nan')):.6f}")
                print(f"    SNR cutoff k: {diag.get('snr_cutoff_k', float('nan')):.2f} Å⁻¹")
                print(f"    SNR (diagnostic only): {diag.get('snr', float('nan')):.3f}")
                print(f"    Candidates tested: {diag.get('n_candidates_tested', 0)}")
                print(f"    Expected combinations: {diag.get('n_candidate_combinations_expected', 0)}")
                print(f"    Score tie fraction: {diag.get('score_tie_frac', float('nan')):.3f}")
                print(f"    Near-tied candidates: {diag.get('n_tied_candidates', 0)}")
                print(f"    Eligible kmin zero crossings: {diag.get('n_kmin_zero_crossings_in_range', 0)}")
                print(f"    Eligible kmax zero crossings: {diag.get('n_kmax_zero_crossings_in_range', 0)}")
                print(f"    kmin candidate list: {diag.get('kmin_candidates', [])}")
                print(f"    kmax candidate list: {diag.get('kmax_candidates', [])}")
                print(f"    Zero crossings found: {diag.get('n_zero_crossings_found', 0)}, "
                      f"selected ZC #{diag.get('chosen_zc_index', 0)}")
                print(f"    Selected zero crossings: kmin #{diag.get('chosen_zc_index', 0)}, "
                      f"kmax #{diag.get('chosen_kmax_zc_index', 0)}")
                print(f"    Selection reason: {diag.get('selection_reason', 'unknown')}")
                if diag.get('kmin_at_boundary', False) or diag.get('kmax_at_boundary', False):
                    boundary_flags = []
                    if diag.get('kmin_at_boundary', False):
                        boundary_flags.append("kmin at lower bound")
                    if diag.get('kmax_at_boundary', False):
                        side = diag.get('kmax_boundary_side', 'search')
                        boundary_flags.append(f"kmax at {side} search bound")
                    print(f"    BOUNDARY WARNING: {', '.join(boundary_flags)}")
                print(f"    Nyquist estimate: {diag.get('nyquist_estimate', float('nan')):.2f}")

                top_candidates = diag.get('top_candidates', [])
                if top_candidates:
                    print(f"    Top {len(top_candidates)} candidates by SCORE:")
                    for i, cand in enumerate(top_candidates, 1):
                        prefix = "  ->" if (
                            abs(cand['kmin'] - group.best_kmin) < 0.01 and
                            abs(cand['kmax'] - group.best_kmax) < 0.01
                        ) else "   "
                        print(
                            f"    {prefix} #{i}: kmin={cand['kmin']:.2f}, kmax={cand['kmax']:.2f}, "
                            f"score={cand['score']:.6f}, first_shell_area={cand['first_shell_area']:.6f}, "
                            f"highR_area={cand['highR_area']:.6f}, peak={cand['peak']:.6f}, "
                            f"lowR={cand['lowR']:.6f}, SNR={cand['snr']:.3f}"
                        )

        print("\n" + "-"*80)
        print("  QUICK REFERENCE:")
        print(f"  {'Group':<45s}  {'kmin':>6s}  {'kmax':>6s}  {'Score':>10s}  {'FS Area':>10s}  {'HighR':>10s}")
        print("-"*80)
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                if hasattr(group, 'best_kmin') and hasattr(group, 'best_kmax'):
                    diag = getattr(group, 'ft_diagnostics', {})
                    print(
                        f"    {proj_name}.{group_name:<40s}  "
                        f"{group.best_kmin:>6.2f}  {group.best_kmax:>6.2f}  "
                        f"{diag.get('score', float('nan')):>10.6f}  "
                        f"{diag.get('first_shell_area', float('nan')):>10.6f}  "
                        f"{diag.get('high_r_area', float('nan')):>10.6f}"
                    )
                else:
                    print(f"    {proj_name}.{group_name:<40s}  (no FT data)")
        print("="*80 + "\n")
    ...
    def _default_global_e0(self):
        """Return the default shared e0 value used across fitting workflows."""
        return -3.3

    def _resolve_e0_seed(self, prev_result=None, vary_e0=False, e0_value=None):
        """Resolve the e0 seed used to initialize or fix fit parameters."""
        target_e0 = self._default_global_e0() if e0_value is None else float(e0_value)
        if vary_e0 and prev_result is not None and hasattr(prev_result, 'params'):
            for pname in ('e0', 'del_e0'):
                if pname in prev_result.params:
                    try:
                        return float(prev_result.params[pname].value)
                    except Exception:
                        pass
        return target_e0

    def build_params_for_iteration(self, iteration, prev_result=None, So_2=0.94,
                                   e0_value=None, vary_e0=False):
        """
        ...
        seed_e0 = self._resolve_e0_seed(
            prev_result=prev_result,
            vary_e0=vary_e0,
            e0_value=e0_value,
        )
        params.del_e0 = param(seed_e0, vary=vary_e0, min=-15, max=15)
        # N_I (Pb-I coordination): chemically bounded to [1.5, 6.0]. Tight
        # bound prevents N_I collapse when Pb-S is added in iter 3 (Pb-S
        # competes with Pb-I for amplitude when sigma2_I/delr_I drift).
        params.n_PbI = param(3.0, vary=True, min=1.5, max=6.0)
        ...
        if prev_result is not None and hasattr(prev_result, 'params'):
            ...
            for name in ['amp', 'n_PbI', 'pbi_sig2', 'pbi_delr']:
                if name in prev_result.params:
                    ...
                    if hasattr(params, name) and hasattr(params[name], 'value'):
                        ...
                        setattr(params, name, param(value, 
                                                ...
                                                min=params[name].min, 
                                                max=params[name].max))
        params.del_e0.value = seed_e0
        params.del_e0.vary = vary_e0
    ...
    def build_linked_params(self, iteration, prev_result=None, s02_value=0.78, 
                            ...
                            e0_value=None, use_pb_o_aimd_single_shell=False,
                            pb_o_sigma2_mode='restrain',
                            pb_o_sigma2_restraint_strength=1.0,
                            pb_o_model='single',
                            pb_o_split_reff_O1=None,
                            pb_o_split_reff_O2=None):
        """
        ...
        Parameters:
            ...
            use_pb_o_aimd_single_shell (bool): If True, enable the minimal-change
                Pb-O treatment with AIMD-informed sigma2 and a Pb-O-only third cumulant.
            pb_o_sigma2_mode (str): ``'restrain'`` (default) or ``'fix'`` for the
                Pb-O sigma2 treatment when ``use_pb_o_aimd_single_shell`` is enabled.
            pb_o_sigma2_restraint_strength (float): Strength of the Pb-O sigma2
                restraint in ``'restrain'`` mode. Larger values are tighter.
        ...
        params.e0 = param(seed_e0, vary=not fix_e0, min=-15, max=15)
        
        # --- Coordination number parameters ---
        # N_I: iodine coordination (independent). Bounded to [1.5, 6.0]:
        # chemically reasonable for Pb halide solutions and prevents N_I
        # collapse when later iterations add competing shells (e.g. Pb-S).
        params.N_I = param(3.0, vary=True, min=1.5, max=6.0)
            ...
            for old_name, new_name in param_mapping.items():
                if old_name in prev_params and hasattr(params, new_name):
                    try:
                        ...
                        pass
        params.e0.value = seed_e0
        params.e0.vary = not fix_e0
        ...
        if use_pb_o_aimd_single_shell:
            prev_c3_o = 0.0
            if prev_result is not None and hasattr(prev_result, 'params') and 'c3_o' in prev_result.params:
                prev_c3_o = prev_result.params['c3_o'].value

            # Minimal-change single-shell Pb-O treatment:
            # keep one Pb-O shell, use the AIMD-derived sigma2 only for that shell,
            # and add a Pb-O-only third cumulant for the mild asymmetry in the Pb-O distribution.
            self._apply_pb_o_aimd_sigma2_constraint(
                params,
                sigma2_param_name='sigma2_O',
                sigma2_value=getattr(params.sigma2_O, 'value', 0.055225),
                third_param_name='c3_o',
                c3_value=prev_c3_o,
                pb_o_sigma2_mode=pb_o_sigma2_mode,
                pb_o_sigma2_restraint_strength=pb_o_sigma2_restraint_strength,
                sigma2_min=0.0,
                sigma2_max=0.08,
                create_param=param,
            )

        # Split two-shell Pb-O parameters: replace the single N_O / sigma2_O / delr_O
        # with N_O1, N_O2, delr_O1, split_O, sigma2_O_fixed. No third cumulant.
        pb_o_model_norm = str(pb_o_model or 'single').lower()
        if pb_o_model_norm == 'split_two_shell':
            if use_pb_o_aimd_single_shell:
                raise ValueError(
                    "pb_o_model='split_two_shell' is incompatible with "
                    "use_pb_o_aimd_single_shell=True (no third cumulant in split model)."
                )
            self._add_split_pb_o_parameters(
                params,
                reff_O1=pb_o_split_reff_O1 if pb_o_split_reff_O1 is not None else 2.42,
                reff_O2=pb_o_split_reff_O2 if pb_o_split_reff_O2 is not None else 2.76,
                create_param=param,
                naming='linked',
                n1_init=getattr(getattr(params, 'N_O', None), 'value', 2.0) / 2.0
                       if hasattr(params, 'N_O') else 2.0,
                n2_init=getattr(getattr(params, 'N_O', None), 'value', 2.0) / 2.0
                       if hasattr(params, 'N_O') else 2.0,
                delr1_init=getattr(getattr(params, 'delr_O', None), 'value', 0.0)
                          if hasattr(params, 'delr_O') else 0.0,
                prev_result=prev_result,
            )
            # Drop the legacy single-shell Pb-O parameters so they cannot leak
            # into path expressions. If deletion fails (e.g. on a param_group
            # that does not allow attribute removal), force them to safe fixed
            # values so they cannot vary or perturb the fit.
            for _legacy in ('N_O', 'sigma2_O', 'delr_O', 'c3_o'):
                if hasattr(params, _legacy):
                    deleted = False
                    try:
                        delattr(params, _legacy)
                        deleted = True
                    except Exception:
                        deleted = False
                    if not deleted:
                        try:
                            p_legacy = getattr(params, _legacy)
                            if _legacy == 'sigma2_O':
                                p_legacy.value = self.PB_O_SPLIT_SIGMA2
                            elif _legacy == 'N_O':
                                # Mirror composite for any stale references.
                                n1v = getattr(getattr(params, 'N_O1', None), 'value', 0.0)
                                n2v = getattr(getattr(params, 'N_O2', None), 'value', 0.0)
                                p_legacy.value = float(n1v) + float(n2v)
                            else:
                                p_legacy.value = 0.0
                            p_legacy.vary = False
                        except Exception:
        ...
        print(f"    N_I       = {params.N_I.value:.3f}")
        print(f"    sigma2_I  = {params.sigma2_I.value:.4f}")
        print(f"    delr_I    = {params.delr_I.value:.4f}")
        if pb_o_model_norm == 'split_two_shell':
            n1 = params.N_O1.value if hasattr(params, 'N_O1') else float('nan')
            n2 = params.N_O2.value if hasattr(params, 'N_O2') else float('nan')
            sig_fix = (params.sigma2_O_fixed.value
                       if hasattr(params, 'sigma2_O_fixed') else self.PB_O_SPLIT_SIGMA2)
            d1 = params.delr_O1.value if hasattr(params, 'delr_O1') else float('nan')
            sp = params.split_O.value if hasattr(params, 'split_O') else float('nan')
            print(f"    [Pb-O split-shell model]")
            print(f"    N_O1      = {n1:.3f}")
            print(f"    N_O2      = {n2:.3f}")
            print(f"    N_O_total = {n1 + n2:.3f}")
            print(f"    sigma2_O  = {sig_fix:.4f} (FIXED)")
            print(f"    delr_O1   = {d1:.4f}")
            print(f"    split_O   = {sp:.4f}")
        else:
            if hasattr(params, 'N_O'):
                print(f"    N_O       = {params.N_O.value:.3f}")
            if hasattr(params, 'sigma2_O'):
                sig_o_vary = getattr(params.sigma2_O, 'vary', True)
                tag = ' (FIXED)' if not sig_o_vary else ''
                print(f"    sigma2_O  = {params.sigma2_O.value:.4f}{tag}")
            if hasattr(params, 'delr_O'):
                print(f"    delr_O    = {params.delr_O.value:.4f}")
            if hasattr(params, 'c3_o'):
                print(f"    c3_o      = {params.c3_o.value:.5f}")
    ...
    def _validate_pb_o_sigma2_mode(self, pb_o_sigma2_mode):
        """Validate the Pb-O sigma2 handling mode."""
        mode = str(pb_o_sigma2_mode).strip().lower()
        if mode not in ('restrain', 'fix'):
            raise ValueError(
                f"Unsupported pb_o_sigma2_mode '{pb_o_sigma2_mode}'. "
                "Use 'restrain' or 'fix'."
            )
        return mode

    def _is_pb_o_single_scattering_path(self, path, path_info=None):
        """Return True only for a single-scattering Pb-O path."""
        if path_info is None:
            path_info = self.identify_path_elements(path)
        if path_info.get('is_ms', False):
            return False
        primary = str(path_info.get('primary', '') or '').upper()
        if primary == 'O':
            return True
        label = str(getattr(path, 'label', '') or '').upper()
        return ('PB-O' in label or 'PB_O' in label) and 'MS' not in label

    def _build_pb_o_sigma2_penalty_terms(self, sigma2_param_name, enabled=False,
                                         pb_o_sigma2_mode='restrain',
                                         pb_o_sigma2_restraint_strength=1.0):
        """Create optional Pb-O sigma2 restraint terms for feffit."""
        if not enabled:
            return []
        mode = self._validate_pb_o_sigma2_mode(pb_o_sigma2_mode)
        if mode == 'fix':
            return []
        strength = float(pb_o_sigma2_restraint_strength)
        if strength <= 0:
            raise ValueError("pb_o_sigma2_restraint_strength must be > 0.")
        return [{
            'param': sigma2_param_name,
            'target': 0.055225,
            'base_width': 0.002,
            'strength': strength,
            'label': 'Pb-O AIMD sigma2 restraint',
        }]

    def _apply_pb_o_aimd_sigma2_constraint(self, params, sigma2_param_name,
                                           sigma2_value=None,
                                           third_param_name='c3_o',
                                           c3_value=0.0,
                                           pb_o_sigma2_mode='restrain',
                                           pb_o_sigma2_restraint_strength=1.0,
                                           sigma2_min=0.0, sigma2_max=0.08,
                                           c3_min=-0.01, c3_max=0.01,
                                           create_param=param):
        """Apply the Pb-O AIMD-informed sigma2 and Pb-O third cumulant."""
        mode = self._validate_pb_o_sigma2_mode(pb_o_sigma2_mode)
        target_sigma2 = 0.055225
        if float(pb_o_sigma2_restraint_strength) <= 0:
            raise ValueError("pb_o_sigma2_restraint_strength must be > 0.")

        if sigma2_value is None:
            sigma2_value = target_sigma2
        sigma2_value = float(np.clip(sigma2_value, sigma2_min, sigma2_max))
        c3_value = float(np.clip(c3_value, c3_min, c3_max))

        # Pb-O sigma2 is informed by AIMD here using the explicit target
        # C2 = sigma^2 = 0.055225 A^2 for the single Pb-O shell.
        setattr(
            params,
            sigma2_param_name,
            create_param(
                target_sigma2 if mode == 'fix' else sigma2_value,
                vary=(mode != 'fix'),
                min=sigma2_min,
                max=sigma2_max,
            )
        ...
        # The Pb-O third cumulant is included only for this path to capture
        # mild asymmetry while keeping the Pb-O treatment as a single-shell,
        # minimal-change model instead of splitting the shell.
        setattr(
            params,
            third_param_name,
            create_param(c3_value, vary=True, min=c3_min, max=c3_max)
        )
        return params

    def _configure_pb_o_single_shell_path(self, path, amplitude_expr, sigma2_expr,
                                          deltar_expr, use_pb_o_aimd_single_shell=False,
                                          third_expr='c3_o'):
        """Configure the single-scattering Pb-O path expressions."""
        path.s02 = amplitude_expr
        path.sigma2 = sigma2_expr
        path.deltar = deltar_expr
        if use_pb_o_aimd_single_shell:
            # Minimal-change single-shell Pb-O treatment:
            # keep one Pb-O path, use the AIMD-informed sigma2 for that path,
            # and attach a Pb-O-only third cumulant for mild asymmetry.
            path.third = third_expr
        else:
            path.third = 0.0
        return path

    # ------------------------------------------------------------------
    # Split two-shell Pb-O model
    # ------------------------------------------------------------------
    # Split Pb-O shell model:
    # Two Pb-O paths represent two radial oxygen populations.
    # sigma^2 is fixed at the AIMD-derived disorder estimate (0.025 A^2).
    # The final fitted Pb-O distances are constrained to differ by split_O,
    # with 0.30 <= split_O <= 0.38 A.
    # N_O1 and N_O2 are independent, but N_O_total = N_O1 + N_O2 is checked
    # for physical reasonableness post-fit.
    PB_O_SPLIT_SIGMA2 = 0.025          # A^2, AIMD-informed fixed disorder
    PB_O_SPLIT_MIN = 0.30               # A
    PB_O_SPLIT_MAX = 0.38               # A
    PB_O_SPLIT_TARGET = 0.34            # A
    PB_O_SPLIT_N_MAX = 6.0              # individual coordination upper bound
    PB_O_SPLIT_NTOTAL_MAX = 8.0         # total coordination upper bound
    # AIMD-derived ratio: N_O1 (short) = PB_O_AIMD_RATIO * N_O2 (long).
    # When set, N_O1 becomes a derived parameter (expr) and only N_O2 varies.
    PB_O_AIMD_RATIO = 2.6

    def _select_split_pb_o_paths(self, candidate_infos,
                                 min_split=None, max_split=None,
                                 target_split=None,
                                 reff_min=2.1, reff_max=2.9):
        """
        Pick a (short, long) Pb-O path pair whose Reff separation lies in the
        physically allowed window [min_split, max_split], preferring a separation
        closest to target_split.

        Parameters
        ----------
        candidate_infos : list of dict
            FEFF path-info dicts as produced by ``_get_candidate_paths``.
        min_split, max_split, target_split : float or None
            Override the class-level defaults if not None.
        reff_min, reff_max : float
            Pre-filter window applied to candidate Reff values.

        Returns
        -------
        (info_O1, info_O2, reff_O1, reff_O2, diagnostic) or
        (None, None, None, None, diagnostic) on failure.
        """
        min_split = self.PB_O_SPLIT_MIN if min_split is None else float(min_split)
        max_split = self.PB_O_SPLIT_MAX if max_split is None else float(max_split)
        target_split = self.PB_O_SPLIT_TARGET if target_split is None else float(target_split)

        ss = [c for c in candidate_infos
              if c.get('path_type', '').lower() == 'ss'
              and reff_min <= float(c.get('reff', 0.0)) <= reff_max]
        ss.sort(key=lambda c: float(c.get('reff', np.inf)))

        diagnostic = {
            'n_candidates_in_window': len(ss),
            'reff_window': (reff_min, reff_max),
            'min_split': min_split,
            'max_split': max_split,
        ...
        if len(ss) < 2:
            diagnostic['reason'] = (
                f"Only {len(ss)} Pb-O SS candidate(s) within {reff_min:.2f}-{reff_max:.2f} A; "
                "split-shell model requires at least 2."
            )
            return None, None, None, None, diagnostic

        best = None  # (score, info_O1, info_O2, reff_O1, reff_O2, sep)
        for i in range(len(ss)):
            for j in range(i + 1, len(ss)):
                r1 = float(ss[i].get('reff', 0.0))
                r2 = float(ss[j].get('reff', 0.0))
                sep = abs(r2 - r1)
                diagnostic['considered_pairs'] += 1
                if not (min_split <= sep <= max_split):
                    continue
                score = abs(sep - target_split)
                if (best is None) or (score < best[0]):
                    if r1 <= r2:
                        best = (score, ss[i], ss[j], r1, r2, sep)
                    else:
                        best = (score, ss[j], ss[i], r2, r1, sep)

        if best is None:
            diagnostic['reason'] = (
                f"No Pb-O SS pair with Reff separation in [{min_split:.2f}, {max_split:.2f}] A. "
                f"Considered {diagnostic['considered_pairs']} pair(s) from {len(ss)} candidates."
            )
            return None, None, None, None, diagnostic

        _, info_O1, info_O2, reff_O1, reff_O2, sep = best
        diagnostic['reason'] = (
            f"Selected pair Reff_O1={reff_O1:.3f} A, Reff_O2={reff_O2:.3f} A, "
            f"separation={sep:.3f} A (target={target_split:.2f} A)."
        )
        diagnostic['selected_separation'] = sep
        return info_O1, info_O2, reff_O1, reff_O2, diagnostic

    def _split_pb_o_param_names(self, naming='linked'):
        """Return the parameter-name dict for the split Pb-O model."""
        if str(naming).lower() == 'flat':
            # Names used by iterative_fit_all (iter 2).
            return {
                'n1': 'n_PbO1',
                'n2': 'n_PbO2',
                'delr1': 'pbo_delr1',
                'split': 'pbo_split',
                'sigma2_fixed': 'pbo_sig2_fixed',
                's02': 'amp',
            }
        # Names used by build_linked_params / assign_linked_params_to_path (iter >= 3).
        return {
            'n1': 'N_O1',
            'n2': 'N_O2',
            'delr1': 'delr_O1',
            'split': 'split_O',
            'sigma2_fixed': 'sigma2_O_fixed',
            's02': 's02',
        }

    def _build_split_pb_o_delr2_expr(self, reff_O1, reff_O2, names):
        """
        Build the constrained delta-R expression for the long Pb-O path so that
        the fitted Pb-O distances differ by ``split_O``:

            (Reff_O2 + delr_O2) - (Reff_O1 + delr_O1) = split_O
        """
        return (
            f"{float(reff_O1):.8f} + {names['delr1']} + {names['split']} "
            f"- {float(reff_O2):.8f}"
        )

    def _add_split_pb_o_parameters(self, params, reff_O1, reff_O2,
                                   create_param,
                                   naming='linked',
                                   n1_init=2.0, n2_init=2.0,
                                   delr1_init=0.0, split_init=None,
                                   prev_result=None):
        """
        Add split Pb-O parameters to ``params`` and return the names used.

        Adds, with sensible bounds:
          N_O1   : guess in [0, PB_O_SPLIT_N_MAX]
          N_O2   : guess in [0, PB_O_SPLIT_N_MAX]
          delr_O1: guess in [-0.10, 0.10]
          split_O: guess in [PB_O_SPLIT_MIN, PB_O_SPLIT_MAX]
          sigma2_O_fixed: param(PB_O_SPLIT_SIGMA2, vary=False)

        ``naming`` selects the flat (iter 2) or linked (iter >= 3) parameter
        names. ``prev_result`` is consulted to seed initial values when present.
        """
        names = self._split_pb_o_param_names(naming=naming)
        if split_init is None:
            split_init = self.PB_O_SPLIT_TARGET

        # AIMD ratio constraint: N_O1 = ratio * N_O2 (short-to-long ratio from AIMD).
        # Only N_O2 is varied; N_O1 is a derived parameter. Set ratio <= 0 to disable.
        aimd_ratio = float(getattr(self, 'PB_O_AIMD_RATIO', 0.0) or 0.0)
        use_aimd_ratio = aimd_ratio > 0.0

        # Seed from previous fit if available (handles both naming conventions).
        def _seed(value, *aliases):
            if prev_result is None or not hasattr(prev_result, 'params'):
                return value
            for a in aliases:
                if a in prev_result.params:
                    try:
                        return float(prev_result.params[a].value)
                    except Exception:
                        pass
            return value

        n1_init = float(np.clip(
            _seed(n1_init, names['n1'], 'N_O1', 'n_PbO1'),
            0.0, self.PB_O_SPLIT_N_MAX,
        ))
        n2_init = float(np.clip(
            _seed(n2_init, names['n2'], 'N_O2', 'n_PbO2'),
            0.0, self.PB_O_SPLIT_N_MAX,
        ))
        delr1_init = float(np.clip(
            _seed(delr1_init, names['delr1'], 'delr_O1', 'pbo_delr1'),
            -0.10, 0.10,
        ))
        split_init = float(np.clip(
            _seed(split_init, names['split'], 'split_O', 'pbo_split'),
            self.PB_O_SPLIT_MIN, self.PB_O_SPLIT_MAX,
        ))

        if use_aimd_ratio:
            # Re-seed N_O2 from total guess so N_O1 = ratio * N_O2 reproduces the
            # original short+long sum: total = (1+ratio)*N_O2  =>  N_O2 = total/(1+ratio).
            total_guess = n1_init + n2_init
            n2_max = float(self.PB_O_SPLIT_N_MAX) / aimd_ratio
            n2_init = float(np.clip(total_guess / (1.0 + aimd_ratio), 0.0, n2_max))
            setattr(params, names['n2'],
                    create_param(n2_init, vary=True, min=0.0, max=n2_max))
            # N_O1 derived from N_O2 via AIMD ratio.
            setattr(params, names['n1'],
                    create_param(expr=f"{aimd_ratio:.6f} * {names['n2']}"))
        else:
            setattr(params, names['n1'],
                    create_param(n1_init, vary=True, min=0.0, max=self.PB_O_SPLIT_N_MAX))
            setattr(params, names['n2'],
                    create_param(n2_init, vary=True, min=0.0, max=self.PB_O_SPLIT_N_MAX))
        setattr(params, names['delr1'],
                create_param(delr1_init, vary=True, min=-0.10, max=0.10))
        setattr(params, names['split'],
                create_param(split_init, vary=True,
                             min=self.PB_O_SPLIT_MIN, max=self.PB_O_SPLIT_MAX))
        setattr(params, names['sigma2_fixed'],
                create_param(self.PB_O_SPLIT_SIGMA2, vary=False,
                             min=0.0, max=0.10))
        return names

    def _configure_split_pb_o_path(self, path, role, reff_O1, reff_O2,
                                   naming='linked',
                                   e0_name=None, label_prefix=None):
        """
        Configure a Pb-O FEFF path as the short ('O1') or long ('O2') member of
        the split-shell model. Marker attributes are stored on ``path`` so that
        deep-copies in subsequent iterations can recover the role.
        """
        names = self._split_pb_o_param_names(naming=naming)
        role = str(role).upper()
        if role not in ('O1', 'O2'):
            raise ValueError(f"role must be 'O1' or 'O2', got {role!r}")

        path.degen = 1.0
        path.third = 0.0  # No cumulant in split-shell model.

        if role == 'O1':
            n_name = names['n1']
            delr_expr = names['delr1']
        else:
            n_name = names['n2']
            delr_expr = self._build_split_pb_o_delr2_expr(reff_O1, reff_O2, names)

        path.s02 = f"{n_name} * {names['s02']}"
        path.sigma2 = names['sigma2_fixed']
        path.deltar = delr_expr
        if e0_name is not None:
            path.e0 = e0_name

        # Persistent markers used by assign_linked_params_to_path in later iterations.
        path.pb_o_split_role = role
        path.pb_o_split_reff_O1 = float(reff_O1)
        path.pb_o_split_reff_O2 = float(reff_O2)
        path.param_type = 'SS_O_split_O1' if role == 'O1' else 'SS_O_split_O2'

        if label_prefix is not None:
            path.label = f"{label_prefix} ({role})"
        return path

    def _evaluate_split_pb_o_summary(self, fit_result, reff_O1=None, reff_O2=None,
                                     naming='linked'):
        """
        Compute a diagnostic summary of a split Pb-O fit.

        Returns a dict with:
            n_O1, n_O1_err, n_O2, n_O2_err, n_O_total, n_O_total_err,
            delr_O1, split_O, sigma2_O_fixed,
            reff_O1, reff_O2, R_O1, R_O2, split_O_actual,
            warnings (list[str]), passed (bool).
        """
        names = self._split_pb_o_param_names(naming=naming)
        params = getattr(fit_result, 'params', {}) or {}

        def _val(pname, default=np.nan):
            if pname in params:
                try:
                    return float(params[pname].value)
                except Exception:
                    return default
            return default

        def _err(pname, default=0.0):
            if pname in params:
                err = getattr(params[pname], 'stderr', None)
                if err is not None and np.isfinite(err):
                    return float(err)
            return default

        n1, n1e = _val(names['n1']), _err(names['n1'])
        n2, n2e = _val(names['n2']), _err(names['n2'])
        delr1 = _val(names['delr1'])
        split_o = _val(names['split'])
        sig2 = _val(names['sigma2_fixed'], default=self.PB_O_SPLIT_SIGMA2)

        if reff_O1 is None or reff_O2 is None:
            # Not enough info to compute actual Pb-O distances.
            R1 = R2 = split_actual = np.nan
        else:
            R1 = float(reff_O1) + delr1 if np.isfinite(delr1) else np.nan
            R2 = R1 + split_o if np.isfinite(split_o) else np.nan
            split_actual = (R2 - R1) if np.isfinite(R2) and np.isfinite(R1) else np.nan

        n_total = (n1 if np.isfinite(n1) else 0.0) + (n2 if np.isfinite(n2) else 0.0)
        n_total_err = float(np.sqrt(n1e**2 + n2e**2))

        warnings = []
        if np.isfinite(n1) and n1 < 0:
            warnings.append(f"N_O1={n1:.3f} is negative")
        if np.isfinite(n2) and n2 < 0:
            warnings.append(f"N_O2={n2:.3f} is negative")
        if np.isfinite(n_total) and n_total > self.PB_O_SPLIT_NTOTAL_MAX:
            warnings.append(
                f"N_O_total={n_total:.2f} > {self.PB_O_SPLIT_NTOTAL_MAX:.1f}"
            )
        if np.isfinite(split_actual) and (
            split_actual < self.PB_O_SPLIT_MIN - 1e-4
            or split_actual > self.PB_O_SPLIT_MAX + 1e-4
        ):
            warnings.append(
                f"split_O_actual={split_actual:.3f} outside "
                f"[{self.PB_O_SPLIT_MIN:.2f}, {self.PB_O_SPLIT_MAX:.2f}]"
            )
        if np.isfinite(split_o):
            if abs(split_o - self.PB_O_SPLIT_MIN) < 1e-3:
                warnings.append(f"split_O pinned at lower bound {self.PB_O_SPLIT_MIN:.2f}")
            if abs(split_o - self.PB_O_SPLIT_MAX) < 1e-3:
                warnings.append(f"split_O pinned at upper bound {self.PB_O_SPLIT_MAX:.2f}")
        for label, val in (('N_O1', n1), ('N_O2', n2)):
            if np.isfinite(val):
                if abs(val - 0.0) < 1e-3:
                    warnings.append(f"{label} pinned at 0")
                if abs(val - self.PB_O_SPLIT_N_MAX) < 1e-3:
                    warnings.append(f"{label} pinned at {self.PB_O_SPLIT_N_MAX:.0f}")
        if np.isfinite(n1) and n1 < 0.05:
            warnings.append(f"N_O1={n1:.3f} contributes essentially zero amplitude")
        if np.isfinite(n2) and n2 < 0.05:
            warnings.append(f"N_O2={n2:.3f} contributes essentially zero amplitude")

        return {
            'n_O1': n1, 'n_O1_err': n1e,
            'n_O2': n2, 'n_O2_err': n2e,
            'n_O_total': n_total, 'n_O_total_err': n_total_err,
            'delr_O1': delr1,
            'split_O': split_o,
            'split_O_actual': split_actual,
            'sigma2_O_fixed': sig2,
            'reff_O1': reff_O1, 'reff_O2': reff_O2,
            'R_O1': R1, 'R_O2': R2,
            'warnings': warnings,
            'passed': len(warnings) == 0,
        }

    def _print_split_pb_o_summary(self, summary, group_id='', accepted=None,
                                  delta_aicc=None, rfactor=None):
        """Print a human-readable summary of a split Pb-O fit."""
        header = f"Pb-O split-shell model"
        if group_id:
            header += f" [{group_id}]"
        print(f"\n  {header}:")
        print(f"    N_O1       = {summary['n_O1']:.3f} +/- {summary['n_O1_err']:.3f}")
        print(f"    N_O2       = {summary['n_O2']:.3f} +/- {summary['n_O2_err']:.3f}")
        print(f"    N_O_total  = {summary['n_O_total']:.3f} +/- {summary['n_O_total_err']:.3f}")
        print(f"    R_O1       = {summary['R_O1']:.3f} A  (Reff={summary['reff_O1']})")
        print(f"    R_O2       = {summary['R_O2']:.3f} A  (Reff={summary['reff_O2']})")
        print(f"    split_O    = {summary['split_O']:.3f} A "
              f"(actual={summary['split_O_actual']:.3f} A)")
        print(f"    sigma^2    = {summary['sigma2_O_fixed']:.4f} A^2 (fixed)")
        if rfactor is not None:
            print(f"    R-factor   = {rfactor:.6f}")
        if delta_aicc is not None:
            print(f"    Delta AICc = {delta_aicc:+.3f}")
        if summary['warnings']:
            print(f"    sanity     = WARN")
    ...
    def _feffit_with_optional_penalties(self, paramgroup, datasets, penalty_terms=None,
                                        rmax_out=10, path_outputs=True,
                                        fix_unused_variables=True, _larch=None, **kws):
        """Run feffit, optionally adding soft penalty terms to the residual."""
        ...
        from larch.fitting import (correlated_values, eval_stderr, group2params,
                                   params2group, isParameter)
        from larch.xafs.feffit import FeffitDataSet, _feffit_resid
        ...
        if not penalty_terms:
            return feffit(
                ...
                fix_unused_variables=fix_unused_variables,
                _larch=_larch,
                **kws,
            )
        ...
        for p in params.values():
            if p.vary:
                nam = p.name
                if not any([nam.startswith('bkg') and nam.endswith(ds.hashkey) for ds in datasets]):
                    vars.append(nam)
        ...
        if len(vars) > 0:
            if fix_unused_variables:
                for vname in vars:
                    params[vname].vary = False
            else:
                vlist = ', '.join(vars)
                print(f"Feffit Warning: unused variables: {vlist}")

        def penalty_residual(current_params):
            penalties = []
            for term in penalty_terms:
                pname = term.get('param')
                if pname not in current_params:
                    continue
                target = float(term.get('target', 0.0))
                strength = max(float(term.get('strength', 1.0)), 1e-12)
                base_width = max(float(term.get('base_width', 0.002)), 1e-12)
                penalties.append((current_params[pname].value - target) / (base_width / strength))
            return np.asarray(penalties, dtype='float64')

        def residual_with_penalty(current_params, datasets=None):
            data_resid = _feffit_resid(current_params, datasets=datasets)
            extra = penalty_residual(current_params)
            if len(extra) > 0:
                return np.concatenate([data_resid, extra])
            return data_resid

        fit = Minimizer(
            residual_with_penalty,
            params,
        ...
        result = fit.leastsq()
        data_residual = np.concatenate([d._residual(result.params, data_only=True) for d in datasets])
        dat = data_residual
        penalty_resid = penalty_residual(result.params)
        ...
        data_rfactor = (result.residual[:len(dat)] ** 2).sum() / max(1.e-12, (dat ** 2).sum())
        ...
        out = Group(
            ...
            rfactor=data_rfactor,
            aic=aic,
            bic=bic,
            covar=covar,
            penalty_terms=penalty_terms,
            data_residual=data_residual,
            penalty_residual=penalty_resid,
        )
    ...
    def assign_linked_params_to_path(self, path, iteration, fix_delr=False,
        ...
        if is_ms:
            ...
            if 'I' in scatterer_set and 'O' in scatterer_set:
                ...
                path.s02 = 'N_I*2 * s02'
                path.sigma2 = 'sigma2_I * 1.5'  # Assume more disorder for MS paths
            ...
            if primary == 'I':
                ...
                if str(pb_o_model or 'single').lower() == 'split_two_shell':
                    # Use the role mark stored on the path by iter 2 to decide
                    # whether this is the short or long Pb-O shell.
                    role = getattr(path, 'pb_o_split_role', None)
                    reff_O1 = getattr(path, 'pb_o_split_reff_O1', None)
                    reff_O2 = getattr(path, 'pb_o_split_reff_O2', None)
                    if role is None or reff_O1 is None or reff_O2 is None:
                        raise RuntimeError(
                            "Split Pb-O model requires path.pb_o_split_role and "
                            "pb_o_split_reff_O1/O2 attributes; this path was not "
                            "configured by the split-shell builder."
                        )
                    self._configure_split_pb_o_path(
                        path, role=role,
                        reff_O1=reff_O1, reff_O2=reff_O2,
                        naming='linked', e0_name='e0',
                    )
                    # _configure_split_pb_o_path already set param_type.
                else:
                    self._configure_pb_o_single_shell_path(
                        path,
                        'N_O * s02',
                        'sigma2_O',
                        'delr_O' if not fix_delr else 0.0,
                        use_pb_o_aimd_single_shell=use_pb_o_aimd_single_shell,
                    )
        ...
        # Split Pb-O remap: linked Pb-O parameters (N_O / sigma2_O / delr_O) do
        # not exist in split mode. Rewrite references on non-Pb-O paths so that
        # MS / Pb-S / Pb-C / Pb-N expressions still resolve. The Pb-O paths
        # themselves were configured directly above and are skipped here.
        if (str(pb_o_model or 'single').lower() == 'split_two_shell'
                and getattr(path, 'pb_o_split_role', None) is None):
            def _remap(expr):
                if not isinstance(expr, str):
                    return expr
                # Order matters: replace longer tokens first.
                replacements = (
                    ('sigma2_O', 'sigma2_O_fixed'),
                    ('delr_O', 'delr_O1'),
                    ('N_O', '(N_O1 + N_O2)'),
                )
                out = expr
                ...
                return out
            path.s02 = _remap(path.s02)
            path.sigma2 = _remap(path.sigma2)
            if isinstance(path.deltar, str):
                path.deltar = _remap(path.deltar)
    ...
    def robust_feffit(self, params, datasets, max_retries=3, verbose=True,
                      e0_fallback=None, penalty_terms=None):
        """
        ...
        if e0_fallback is None:
            e0_fallback = self._default_global_e0()
        ...
        for retry in range(max_retries + 1):
            try:
                ...
                # Try the fit
                result = self._feffit_with_optional_penalties(
                    fit_params,
                    datasets,
                    penalty_terms=penalty_terms,
                )
        ...
        try:
            fit_params = copy.deepcopy(params)
            for pname in ['s02', 'e0', 'delr_I', 'delr_O', 'delr_S', 'delr_C',
                          'sigma2_I', 'sigma2_O', 'c3_o']:
                if hasattr(fit_params, pname):
                    fit_params[pname].vary = False
    ...
    def fit_with_linked_params(self, group, paths, iteration, prev_result=None,
        ...
        Parameters:
            ...
            verbose (bool): Print diagnostic messages
            use_pb_o_aimd_single_shell (bool): If True, enable the Pb-O-only
                AIMD-informed sigma2 plus third cumulant treatment.
            pb_o_sigma2_mode (str): ``'restrain'`` or ``'fix'`` for the Pb-O sigma2.
            pb_o_sigma2_restraint_strength (float): Restraint strength for the
                Pb-O sigma2 when ``pb_o_sigma2_mode='restrain'``.
        ...
        has_pb_o_single_shell = any(self._is_pb_o_single_scattering_path(path) for path in paths)
        apply_pb_o_aimd = bool(use_pb_o_aimd_single_shell and has_pb_o_single_shell)

        # Detect split Pb-O paths so build_linked_params can seed correct Reff values.
        pb_o_model_norm = str(pb_o_model or 'single').lower()
        split_reff_O1 = split_reff_O2 = None
        if pb_o_model_norm == 'split_two_shell':
            for _p in paths:
                role = getattr(_p, 'pb_o_split_role', None)
                if role == 'O1' and split_reff_O1 is None:
                    split_reff_O1 = float(getattr(_p, 'pb_o_split_reff_O1',
                                                  getattr(_p, 'reff', 2.42)))
                    if split_reff_O2 is None:
                        split_reff_O2 = float(getattr(_p, 'pb_o_split_reff_O2',
                                                      split_reff_O1 + self.PB_O_SPLIT_TARGET))
                elif role == 'O2' and split_reff_O2 is None:
                    split_reff_O2 = float(getattr(_p, 'pb_o_split_reff_O2',
                                                  getattr(_p, 'reff', 2.76)))
                    if split_reff_O1 is None:
                        split_reff_O1 = float(getattr(_p, 'pb_o_split_reff_O1',
                                                      split_reff_O2 - self.PB_O_SPLIT_TARGET))
        ...
        params = self.build_linked_params(
            ...
            e0_value=e0_value,
            use_pb_o_aimd_single_shell=apply_pb_o_aimd,
            pb_o_sigma2_mode=pb_o_sigma2_mode,
            pb_o_sigma2_restraint_strength=pb_o_sigma2_restraint_strength,
            pb_o_model=pb_o_model_norm,
            pb_o_split_reff_O1=split_reff_O1,
            pb_o_split_reff_O2=split_reff_O2,
        )
        
        # Assign parameters to each path
        configured_paths = []
        for path in paths:
            configured_path = self.assign_linked_params_to_path(
                path,
                iteration,
                use_pb_o_aimd_single_shell=apply_pb_o_aimd,
                pb_o_model=pb_o_model_norm,
        ...
        pb_o_penalty_terms = self._build_pb_o_sigma2_penalty_terms(
            sigma2_param_name='sigma2_O',
            enabled=apply_pb_o_aimd,
            pb_o_sigma2_mode=pb_o_sigma2_mode,
            pb_o_sigma2_restraint_strength=pb_o_sigma2_restraint_strength,
        )
        if pb_o_model_norm == 'split_two_shell':
            pb_o_penalty_terms = []  # sigma2 is fixed in the split model.
        ...
        #   R-range: rmin=first |chi(R)| minimum after rbkg (fallback ~1.67 Å),
        #            rmax=4.0 Å
        ...
        # Perform robust fit
        result, success, fixed_params = self.robust_feffit(
            params,
            [dataset],
            verbose=verbose,
            e0_fallback=self._resolve_e0_seed(
                prev_result=prev_result,
                vary_e0=vary_e0,
                e0_value=e0_value,
            ),
            penalty_terms=pb_o_penalty_terms,
        )
        ...
        if success and result is not None:
            if verbose:
                ...
                for pname in ['N_I', 'N_O', 'N_O1', 'N_O2', 'sigma2_I', 'sigma2_O',
                              'sigma2_O_fixed', 'split_O', 'delr_O1', 'c3_o', 'e0']:
    ...
    def iterative_fit_linked_all(self, feff_category, iteration, feff_base_dir, 
                                ...
                                force_add_path=False, force_add_min_paths=1,
                                physical_sanity_mode='all',
        ...
        Parameters:
            ...
            force_add_path (bool): If True, allow adding the best available candidate(s)
                even when the normal AICc/R-factor acceptance threshold is not met.
            force_add_min_paths (int): Minimum number of paths to force-add when
                ``force_add_path`` is enabled.
            physical_sanity_mode (str): One of ``'all'``, ``'candidate'``,
                ``'final'``, or ``'off'`` to control sanity-check enforcement.
            require_valid_aicc (bool): If True, reject candidate fits with invalid AICc.
            allow_rfactor_fallback (bool): If True, permit R-factor improvement to
                accept a candidate even when AICc does not clear the threshold.
            reject_on_bound_pinning (bool): If True, reject candidate/final fits when
                path-linked parameters are pinned near bounds.
        ...
        print(f"Sanity mode: {physical_sanity_mode}")
        print(f"Require valid AICc: {require_valid_aicc}")
        print(f"Allow R-factor fallback: {allow_rfactor_fallback}")
        print(f"Force add path(s): {force_add_path} (min={force_add_min_paths})")
        print(f"Reject bound pinning: {reject_on_bound_pinning}")
        print(f"Vary e0: {vary_e0} (default fixed e0 = {self._default_global_e0():.3f} eV)")
        if use_pb_o_aimd_single_shell:
            print(
                "Pb-O AIMD single-shell: "
                f"mode={self._validate_pb_o_sigma2_mode(pb_o_sigma2_mode)}, "
                "sigma2 target=0.055225 A^2, "
                f"restraint strength={pb_o_sigma2_restraint_strength}"
            )
        print(f"{'='*70}")
        ...
        total_processed = 0
        total_improved = 0
        physical_sanity_mode = str(physical_sanity_mode).lower()
        valid_sanity_modes = {'all', 'candidate', 'final', 'off'}
        if physical_sanity_mode not in valid_sanity_modes:
            raise ValueError(
                f"Invalid physical_sanity_mode '{physical_sanity_mode}'. "
                f"Expected one of {sorted(valid_sanity_modes)}."
            )
        check_candidate_sanity = physical_sanity_mode in ('all', 'candidate')
        check_final_sanity = physical_sanity_mode in ('all', 'final')
        ...
        def carry_forward_group_state(group, group_id, prev_fit, prev_paths, reason):
            """Persist the previous iteration state into the current iteration slot."""
            try:
                carried_fit = copy.deepcopy(prev_fit)
            except Exception:
                carried_fit = prev_fit
            try:
                carried_paths = copy.deepcopy(prev_paths)
            except Exception:
                carried_paths = list(prev_paths)

            setattr(group, f'fit_result_iter{iteration}', carried_fit)
            setattr(group, f'paths_iter{iteration}', carried_paths)

            prev_aicc = self._compute_aicc(prev_fit)
            results[group_id] = {
                'rfactor': prev_fit.rfactor,
                'improvement': 0.0,
                'improved': False,
                'paths_added': 0,
                'aicc_old': prev_aicc.get('aicc', float('nan')),
                'aicc_new': prev_aicc.get('aicc', float('nan')),
                'delta_aicc': 0.0,
                'carried_forward': True,
                'reason': reason,
            }
            if verbose:
                print(f"    Carrying forward iteration {iteration-1} state to iteration {iteration}: {reason}")
        ...
        if not candidate_paths:
            for proj_name, project in self.projects.items():
                for group_name, group in project.groups.items():
                    ...
                    carry_forward_group_state(
                        group, group_id, prev_fit, prev_paths,
                        f'no candidate {feff_category} paths found'
                    )
            return results
        ...
        for proj_name, project in self.projects.items():
            ...
            for group_name, group in project.groups.items():
                ...
                # get_transform_for_group() returns TransformGroup with:
                #   k-range: kmin, kmax from group's best_kmin/best_kmax or defaults (3.0, 12.0)
                #   R-range: rmin=first |chi(R)| minimum after rbkg
                #            (fallback ~1.67 Å), rmax=4.0 Å
                trans = self.get_transform_for_group(group)
                ...
                for cand in filtered[:max_paths_to_add * 2]:
                    ...
                    try:
                        ...
                        sane, reason = True, 'sanity check disabled'
                        if check_candidate_sanity:
                        ...
                        if reject_on_bound_pinning:
                            delr_names, sig2_names, n_names = self._candidate_param_names_from_path(new_path)
                            if self._fit_has_bad_bounds(
                                result,
                                delr_names=delr_names,
                                sig2_names=sig2_names,
                                n_names=n_names
                            ):
                                cand_record['rejection_reason'] = 'parameter pinned near bound'
                                test_results.append(cand_record)
                                if verbose:
                                    print(f"      {cand['label'][:35]:35s} BOUND FAIL")
                                continue

                        # Compute AICc for candidate model
                        cand_aicc = self._compute_aicc(result)
                        cand_record['aicc'] = cand_aicc['aicc']
                        cand_record['aicc_valid'] = cand_aicc['valid']
                        if require_valid_aicc and not cand_aicc['valid']:
                        ...
                        aicc_improved = False
                        if not prev_aicc_info['valid'] and cand_aicc['valid']:
                            aicc_improved = True
                        elif prev_aicc_info['valid'] and cand_aicc['valid']:
                            aicc_improved = (prev_aicc_info['aicc'] - cand_aicc['aicc']) >= aicc_improvement_threshold
                        rfactor_improved = improvement > 0
                        cand_record['metric_accept'] = aicc_improved or (allow_rfactor_fallback and rfactor_improved)
                        if aicc_improved:
                            cand_record['accepted_by'] = 'aicc'
                        elif allow_rfactor_fallback and rfactor_improved:
                            cand_record['accepted_by'] = 'rfactor'
                ...
                viable_candidates = [
                    ...
                    if tr['result'] is not None and not tr['rejection_reason']
                ]
                viable_candidates.sort(
                    key=lambda x: (
                        x['aicc'] if np.isfinite(x['aicc']) else float('inf'),
                        x['rfactor'],
                        x['path_info'].get('reff', float('inf'))
                    )
                )

                metric_candidates = [tr for tr in viable_candidates if tr['metric_accept']]
                paths_to_add = []
                for sc in metric_candidates:
                ...
                forced_added = 0
                if force_add_path:
                    min_forced = max(0, min(int(force_add_min_paths), int(max_paths_to_add)))
                    if len(paths_to_add) < min_forced:
                        for sc in viable_candidates:
                            if sc in paths_to_add:
                                continue
                            sc['forced'] = True
                            sc['accepted_by'] = sc.get('accepted_by') or 'forced'
                            paths_to_add.append(sc)
                            forced_added += 1
                            if len(paths_to_add) >= min_forced or len(paths_to_add) >= max_paths_to_add:
                                break

                if not paths_to_add:
                    best_reason = 'no candidates passed AICc threshold'
                    if metric_candidates:
                        best_delta = metric_candidates[0]['delta_aicc']
                        ...
                        best_reason = (
                            f"best viable candidate had AICc={best['aicc']:+.2f}, "
                            f"R={best['rfactor']:.6f}"
                        )
                    elif test_results:
                    ...
                    carry_forward_group_state(group, group_id, prev_fit, prev_paths, best_reason)
                    continue
                ...
                if not success or final_result is None:
                    ...
                    carry_forward_group_state(
                        group, group_id, prev_fit, prev_paths,
                        'final fit did not converge'
                    )
                    continue

                # Final sanity and AICc gate
                final_sane, final_reason = True, 'sanity check disabled'
                if check_final_sanity:
                ...
                final_improvement = prev_rfactor - final_result.rfactor
                forced_accept = any(pa.get('forced', False) for pa in paths_to_add)

                if not final_sane:
                    print(f"    REJECTED final fit: sanity failed ({final_reason})")
                    carry_forward_group_state(
                        group, group_id, prev_fit, prev_paths,
                        f'final sanity: {final_reason}'
                    )
                    continue
                ...
                if reject_on_bound_pinning:
                    ...
                    for path in final_paths:
                        delr_names, sig2_names, n_names = self._candidate_param_names_from_path(path)
                        if self._fit_has_bad_bounds(
                            final_result,
                            delr_names=delr_names,
                            sig2_names=sig2_names,
                            ...
                            break
                    if bound_pinned:
                        print(f"    REJECTED final fit: parameter pinned near bound")
                        carry_forward_group_state(
                            group, group_id, prev_fit, prev_paths,
                            'bound pinning'
                        )
                ...
                final_metric_accept = False
                if not prev_aicc_info['valid'] and final_aicc['valid']:
                    final_metric_accept = True
                elif prev_aicc_info['valid'] and final_aicc['valid']:
                    final_metric_accept = (prev_aicc_info['aicc'] - final_aicc['aicc']) >= aicc_improvement_threshold
                if allow_rfactor_fallback and final_improvement > 0:
                    final_metric_accept = True

                if False and final_aicc['valid'] and prev_aicc_info['valid']:
                ...
                if require_valid_aicc and not final_aicc['valid'] and not forced_accept:
                    print(f"    REJECTED final fit: invalid AICc ({final_aicc.get('reason', 'unknown')})")
                    carry_forward_group_state(
                        group, group_id, prev_fit, prev_paths,
                        f"AICc invalid: {final_aicc.get('reason', 'unknown')}"
                    )
                    continue

                if not final_metric_accept and not forced_accept:
                    print(f"    REJECTED final fit: no AICc or R-factor improvement")
                    carry_forward_group_state(
                        group, group_id, prev_fit, prev_paths,
                ...
                accept_label = "ACCEPTED (forced)" if forced_accept and not final_metric_accept else "ACCEPTED"
                print(f"\n    {accept_label}: R-factor {prev_rfactor:.6f} -> {final_result.rfactor:.6f}")
        ...
        model_selection = f"AICc (threshold={aicc_improvement_threshold})"
        if allow_rfactor_fallback:
            model_selection += " with R-factor fallback"
        if force_add_path:
            model_selection += f", forced add min={force_add_min_paths}"
    ...
    def _get_fit_residual_array(self, fit_result):
        ...
        Reconstructs the residual from the first dataset's data and model
        chi(R) whenever possible so AIC/AICc comparisons stay on the same
        footing for both standard ``feffit`` results and custom penalized fits.
        Falls back to stored residual arrays only if dataset outputs are not
        available.
        ...
        if fit_result is None:
            return None
        # Reconstruct from dataset first so model-selection metrics compare the
        # same data-space residual even when different fitting backends store
        # residual arrays differently.
        try:
            ...
            pass
        if hasattr(fit_result, 'data_residual') and fit_result.data_residual is not None:
            return np.asarray(fit_result.data_residual)
            ...
            if 'sigma2' in pname.lower():
                ...
                if p.value < 0:
                    ...
                    if verbose:
                        print(f'    Sanity FAIL: {reason}')
                    return False, reason
                # Only apply bound-hugging rejection to true fit variables.
                # Path-output sigma2 values created internally by feffit often
                # mirror the public parameter expressions and can sit at a path
                # bound without meaning the user-facing sigma2 parameter is bad.
                if getattr(p, 'vary', False) and self._parameter_is_near_bound(p, frac=0.01):
    ...
    def _candidate_param_names_from_path(self, path):
        ...
        role_map = {
            'SS_I':   (['delr_I', 'pbi_delr'], ['sigma2_I', 'pbi_sig2'], ['N_I', 'n_PbI']),
            'SS_O':   (['delr_O', 'pbo_delr'], ['sigma2_O', 'pbo_sig2'], ['N_O', 'n_PbO']),
            'SS_S':   (['delr_S', 'pbs_delr'], ['sigma2_O', 'pbo_sig2'], ['N_O', 'n_PbO']),
            'SS_C':   (['delr_C', 'pbc_delr'], ['sigma2_O', 'pbo_sig2'], ['N_O', 'n_PbO']),
            'SS_N':   (['delr_O', 'pbo_delr'], ['sigma2_O', 'pbo_sig2'], ['N_O', 'n_PbO']),
            'MS_O_I': (['delr_O', 'delr_I', 'pbo_delr', 'pbi_delr'],
                       ['sigma2_O', 'sigma2_I', 'pbo_sig2', 'pbi_sig2'],
                       ['N_O', 'N_I', 'n_PbO', 'n_PbI']),
            'MS_S_O': (['delr_O', 'delr_S', 'pbo_delr', 'pbs_delr'],
                       ['sigma2_O', 'pbo_sig2'],
                       ['N_O', 'n_PbO']),
            'MS_I':   (['delr_I', 'pbi_delr'], ['sigma2_I', 'pbi_sig2'], ['N_I', 'n_PbI']),
            'MS_O':   (['delr_O', 'pbo_delr'], ['sigma2_O', 'pbo_sig2'], ['N_O', 'n_PbO']),
            'MS_S':   (['delr_S', 'pbs_delr'], ['sigma2_O', 'pbo_sig2'], ['N_O', 'n_PbO']),
        }
    ...
    def replace_large_delr_paths(
        ...
        sanity_scope : {'all', 'candidate', 'final', 'off'}
            Control where physical-sanity checks are enforced.
        require_valid_aicc : bool
            If True, reject candidates whose AICc cannot be computed.
        allow_rfactor_fallback : bool
            If True, allow R-factor improvement alone to accept a candidate
            when AICc does not improve enough.
        reject_on_bound_pinning : bool
            If True, reject fits with path-linked parameters pinned near bounds.
        """
        import os
        import copy
        from pathlib import Path
        ...
        sanity_scope = str(sanity_scope).lower()
        valid_sanity_scopes = {'all', 'candidate', 'final', 'off'}
        if sanity_scope not in valid_sanity_scopes:
            raise ValueError(
                f"Invalid sanity_scope '{sanity_scope}'. "
                f"Expected one of {sorted(valid_sanity_scopes)}."
            )
        check_candidate_sanity = sanity_scope in ('all', 'candidate')
        check_final_sanity = sanity_scope in ('all', 'final')
        ...
        if verbose:
            ...
            print(f"Sanity scope: {sanity_scope}")
            print(f"Vary e0: {vary_e0} (default fixed e0 = {self._default_global_e0():.3f} eV)")
            print(f"Require valid AICc: {require_valid_aicc}")
            print(f"Allow R-factor fallback: {allow_rfactor_fallback}")
            print(f"Reject on bound pinning: {reject_on_bound_pinning}")
            ...
            for group_name, group in project.groups.items():
                ...
                for idx, old_path, role, delr_val in flagged:
                    ...
                    for cand in nearby:
                        try:
                            ...
                            seeded_prev_fit = copy.deepcopy(best_group_fit)
                            target_e0 = self._resolve_e0_seed(
                                prev_result=best_group_fit,
                                vary_e0=vary_e0,
                            )
                            if hasattr(seeded_prev_fit, 'params'):
                                for pname in ('e0', 'del_e0'):
                                    if pname in seeded_prev_fit.params:
                                        try:
                                            seeded_prev_fit.params[pname].value = target_e0
                                        except Exception:
                                            pass

                            refit_result, success, _ = self.fit_with_linked_params(
                                group, trial_paths, iteration, prev_result=seeded_prev_fit,
                                verbose=False, vary_e0=vary_e0
                            )
                            ...
                            # Optional candidate sanity check
                            sane = True
                            sane_reason = 'sanity check disabled'
                            if check_candidate_sanity:
                            ...
                            # Optional bound-pinning rejection for path-linked params
                            if reject_on_bound_pinning:
                                delr_names, sig2_names, n_names = self._candidate_param_names_from_path(trial_paths[idx])
                                if self._fit_has_bad_bounds(
                                    refit_result,
                                    delr_names=delr_names,
                                    sig2_names=sig2_names,
                                    ...
                                    if verbose:
                                        print(f"    Candidate reff={cand['reff']:.3f} rejected: parameter pinned near bound")
                                    continue
                            ...
                            if require_valid_aicc and not cand_aicc['valid']:
                                if verbose:
                                    print(f"    Candidate reff={cand['reff']:.3f} rejected: invalid AICc ({cand_aicc.get('reason', 'unknown')})")
                                continue

                            aicc_improved = False
                            if pre_repl_aicc['valid'] and cand_aicc['valid']:
                                aicc_improved = (
                            ...
                            rfactor_improved = (
                                (best_group_fit.rfactor - refit_result.rfactor) >= improvement_tolerance
                            )

                            if allow_rfactor_fallback:
                                accepted_by_metric = aicc_improved or rfactor_improved
                            else:
                                accepted_by_metric = aicc_improved

                            if not accepted_by_metric:
                                continue

                            # Score: prefer lower AICc, then smaller |ΔR|
                            score_aicc = cand_aicc['aicc'] if cand_aicc['valid'] else float('inf')
                            score = (score_aicc, abs(new_delr), abs(cand['reff'] - target_reff))
                            ...
                            if (local_best_fit is None) or (score < local_best_meta['score']):
                                ...
                                local_best_meta = {
                                    ...
                                    'aicc_valid': cand_aicc['valid'],
                                    'accepted_by': 'aicc' if aicc_improved else 'rfactor',
                                    'sanity_checked': check_candidate_sanity,
                                    'file': cand['filename'],
                                }
                ...
                if accepted_replacements:
                    ...
                    if reject_on_bound_pinning:
                        ...
                        if bound_pinned:
                            if verbose:
                                print(f"  REJECTED all replacements for {group_id}: final fit pinned near bound")
                            results[group_id] = {
                                'replaced': False,
                                'rfactor_before': prev_fit.rfactor,
                                'rfactor_after': prev_fit.rfactor,
                                'n_replaced': 0,
                                'status': 'Bound pinning',
                            }
                            continue
        ...
        model_selection = f"AICc (threshold={aicc_improvement_threshold})"
        if allow_rfactor_fallback:
            model_selection += f" with R-factor fallback (tol={improvement_tolerance:.6f})"
        print(f"  Model selection: {model_selection}")
        print("=" * 80)
        print(f"  ΔR threshold:          {delr_threshold:.3f} Å")
    ...
    def final_shared_e0_fit(self, iteration, verbose=True, plot_results=False, vary_e0=False,
                            pb_o_model='single'):
        """
        ...
        shared_e0 = float(np.mean(e0_values)) if vary_e0 else float(self._default_global_e0())
        ...
        if verbose:
            ...
            if vary_e0:
                print(f"Shared e0 seeded from mean fitted value: {shared_e0:.6f} eV")
            else:
                print(f"Shared e0 fixed to global value: {shared_e0:.6f} eV")
        ...
        for proj_name, group_name, group, prev_fit, paths in fit_groups:
            ...
            params = self.build_linked_params(
                ...
                e0_value=shared_e0,
                pb_o_model=pb_o_model,
                pb_o_split_reff_O1=next(
                    (float(getattr(p, 'pb_o_split_reff_O1'))
                     for p in paths if getattr(p, 'pb_o_split_role', None) == 'O1'),
                    None,
                ),
                pb_o_split_reff_O2=next(
                    (float(getattr(p, 'pb_o_split_reff_O2'))
                     for p in paths if getattr(p, 'pb_o_split_role', None) == 'O2'),
                    None,
                ),
            )
            ...
            for path in copy.deepcopy(paths):
                configured_paths.append(self.assign_linked_params_to_path(
                    path, iteration, pb_o_model=pb_o_model))

            # --- FIT RANGES DEFINED HERE ---
            trans = self.get_transform_for_group(group)
            temp_data = Group(k=group.k, chi=group.chi)
            dataset = feffit_dataset(data=temp_data, pathlist=configured_paths, transform=trans)
            final_fit, success, _ = self.robust_feffit(
                params, [dataset], verbose=False, e0_fallback=shared_e0
            )
    ...
    def _collect_fit_data_for_publication(self):
        ...
        def _extract_reff_from_paths(paths):
            ...
            Returns (reff_I, reff_O, reff_O1, reff_O2). The split-shell reff
            values come from the pb_o_split_reff_O1/O2 attributes that
            _configure_split_pb_o_path persists on each Pb-O FEFF path.
            """
            reff_I = None
            reff_O = None
            reff_O1 = None
            ...
            for path in paths:
                ...
                if (reff is None or reff == 0) and hasattr(path, '_feffdat'):
                    reff = getattr(path._feffdat, 'reff', None)
                # Treat zero/None reff as "not yet evaluated" and skip; the path
                # markers (pb_o_split_reff_O1/O2) below may still be valid.
                ...
                if is_pbi and reff_I is None and reff is not None:
                    reff_I = reff
                elif is_pbo and reff_O is None and reff is not None:
                    reff_O = reff

                # Capture split-shell reff markers on Pb-O paths. As a fallback
                # when the persisted markers are missing, infer them from the
                # path's role attribute combined with its reff: O1 paths carry
                # the short-shell reff, O2 paths carry the long-shell reff.
                if is_pbo:
                    if reff_O1 is None:
                        r1 = getattr(path, 'pb_o_split_reff_O1', None)
                        if r1 is not None:
                            reff_O1 = float(r1)
                    if reff_O2 is None:
                        r2 = getattr(path, 'pb_o_split_reff_O2', None)
                        if r2 is not None:
                            reff_O2 = float(r2)
                    role = getattr(path, 'pb_o_split_role', None)
                    if role == 'O1' and reff_O1 is None and reff is not None:
                        reff_O1 = float(reff)
                    elif role == 'O2' and reff_O2 is None and reff is not None:
                        reff_O2 = float(reff)

                # Stop if we have everything (incl. split markers if available)
                if (reff_I is not None and reff_O is not None
                        and (reff_O1 is not None or not any(
                            getattr(p, 'pb_o_split_role', None) is not None for p in paths))
                        and (reff_O2 is not None or not any(
                            getattr(p, 'pb_o_split_role', None) is not None for p in paths))):
                    break

            return reff_I, reff_O, reff_O1, reff_O2
        ...
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                ...
                reff_I, reff_O, reff_O1, reff_O2 = _extract_reff_from_paths(paths)
                ...
                if reff_O1 is not None:
                    group_data['reff_O1'] = reff_O1
                if reff_O2 is not None:
                    group_data['reff_O2'] = reff_O2
                # When the params indicate a split-shell fit (N_O1 + N_O2),
                # ensure reff_O1/reff_O2 are populated so the bond-length plot
                # can render Pb-O as two distinct shells. Fall back to
                # PB_O_SPLIT_TARGET when path markers were unavailable.
                if ('N_O1' in param_values) and ('N_O2' in param_values):
                    if 'reff_O1' not in group_data:
                        group_data['reff_O1'] = float(reff_O)
                        print(f"  Warning: No reff_O1 marker for {group_id}; "
                              f"using reff_O={reff_O:.3f}")
                    if 'reff_O2' not in group_data:
                        group_data['reff_O2'] = float(group_data['reff_O1']
                                                      + self.PB_O_SPLIT_TARGET)
                        print(f"  Warning: No reff_O2 marker for {group_id}; "
                              f"using reff_O1 + PB_O_SPLIT_TARGET="
                              f"{group_data['reff_O2']:.3f}")
    ...
    def _extract_params_with_errors(self, params):
        ...
        # Split Pb-O override: when N_O1+N_O2 are present in the params, the
        # single-shell N_O / sigma2_O / delr_O values above are placeholders.
        # Replace them with the composite split-shell values so downstream
        # tables and plots show the correct totals.
        n1_alias = next((a for a in ('N_O1', 'n_PbO1') if a in params), None)
        n2_alias = next((a for a in ('N_O2', 'n_PbO2') if a in params), None)
        if n1_alias and n2_alias:
            n1, n2 = params[n1_alias], params[n2_alias]
            n1_err = float(getattr(n1, 'stderr', 0.0) or 0.0)
            n2_err = float(getattr(n2, 'stderr', 0.0) or 0.0)
            result['N_O'] = float(n1.value) + float(n2.value)
            result['N_O_err'] = float(np.sqrt(n1_err**2 + n2_err**2))
            result['N_O1'] = float(n1.value); result['N_O1_err'] = n1_err
            result['N_O2'] = float(n2.value); result['N_O2_err'] = n2_err

            sig_alias = next((a for a in ('sigma2_O_fixed', 'pbo_sig2_fixed') if a in params), None)
            if sig_alias is not None:
                result['sigma2_O'] = float(params[sig_alias].value)
                result['sigma2_O_err'] = 0.0

            delr1_alias = next((a for a in ('delr_O1', 'pbo_delr1') if a in params), None)
            split_alias = next((a for a in ('split_O', 'pbo_split') if a in params), None)
            if delr1_alias is not None:
                d1 = float(params[delr1_alias].value)
                d1_err = float(getattr(params[delr1_alias], 'stderr', 0.0) or 0.0)
                result['delr_O1'] = d1
                result['delr_O1_err'] = d1_err
                # Report short-shell delr as the canonical delr_O
                result['delr_O'] = d1
                result['delr_O_err'] = d1_err
            if split_alias is not None:
                s = float(params[split_alias].value)
                s_err = float(getattr(params[split_alias], 'stderr', 0.0) or 0.0)
                result['split_O'] = s
                result['split_O_err'] = s_err
    ...
    def plot_group_transparency_figure(self, figsize=(12, 10), dpi=300, save_path=None,
        ...
        color_O = '#DC143C'  # Crimson red for Pb-O (single-shell case)
        color_O1 = '#DC143C'  # short Pb-O shell
        color_O2 = '#FF8C00'  # long Pb-O shell
        color_O_total = '#7F1D1D'  # composite total

        # Detect split-shell groups
        split_present = any(('N_O1' in g and 'N_O2' in g)
                            for g in data_collection['groups'])
        ...
        for group_data in data_collection['groups']:
            ...
            if split_present and ('N_O1' in group_data) and ('N_O2' in group_data):
                ax1_twin.errorbar(conc, group_data['N_O1'],
                                  yerr=group_data.get('N_O1_err', 0.0),
                                  fmt='s', markersize=9,
                                  color=color_O1, markeredgecolor='k', markeredgewidth=0.8,
                                  capsize=4, alpha=alpha, zorder=5)
                ax1_twin.errorbar(conc, group_data['N_O2'],
                                  yerr=group_data.get('N_O2_err', 0.0),
                                  fmt='D', markersize=9,
                                  color=color_O2, markeredgecolor='k', markeredgewidth=0.8,
                                  capsize=4, alpha=alpha, zorder=5)
                tot_err = float(np.sqrt(group_data.get('N_O1_err', 0.0) ** 2
                                        + group_data.get('N_O2_err', 0.0) ** 2))
                ax1_twin.errorbar(conc, group_data['N_O1'] + group_data['N_O2'],
                                  yerr=tot_err, fmt='^', markersize=10,
                                  color=color_O_total, markeredgecolor='k', markeredgewidth=0.8,
                                  capsize=4, alpha=alpha * 0.85, zorder=4)
            else:
                ax1_twin.errorbar(conc, group_data['N_O'],
                                  yerr=group_data['N_O_err'],
                                  fmt='s', markersize=10,
        ...
        # In split mode, the Pb-O trend line tracks N_O_total (N_O1 + N_O2),
        # which is the canonical N_O composite.
        conc_means = []
        N_I_means = []
        N_O_means = []
        ...
        if split_present:
            ax1_twin.plot(conc_means, N_O_means, '--', color=color_O_total,
                          alpha=0.5, lw=2, label='_nolegend_')
        else:
            ax1_twin.plot(conc_means, N_O_means, '--', color=color_O,
                          alpha=0.5, lw=2, label='_nolegend_')
        ...
        if split_present:
            ax1_twin.set_ylabel(
                r'$N_{Pb-O_1}\,/\,N_{Pb-O_2}\,/\,N_{Pb-O,total}$',
                fontsize=12, fontweight='bold', color=color_O_total)
            ax1_twin.tick_params(axis='y', labelcolor=color_O_total, labelsize=10)
        ...
        if split_present:
            legend_elements = [
                ...
                Line2D([0], [0], marker='s', color='w', markerfacecolor=color_O1,
                       markeredgecolor='k', markersize=10, label='$N_{Pb-O_1}$ (short)', alpha=1.0),
                Line2D([0], [0], marker='D', color='w', markerfacecolor=color_O2,
                       markeredgecolor='k', markersize=10, label='$N_{Pb-O_2}$ (long)', alpha=1.0),
                Line2D([0], [0], marker='^', color='w', markerfacecolor=color_O_total,
                       markeredgecolor='k', markersize=10, label='$N_{Pb-O,total}$', alpha=1.0),
            ]
        else:
            legend_elements = [
                Line2D([0], [0], marker='o', color='w', markerfacecolor=color_I,
        ...
        for group_data in data_collection['groups']:
            ...
            if split_present and ('N_O1' in group_data) and ('N_O2' in group_data):
                reff_O1_g = float(group_data.get('reff_O1',
                                                 group_data.get('reff_O', 2.42)))
                reff_O2_g = float(group_data.get('reff_O2',
                                                 reff_O1_g + self.PB_O_SPLIT_TARGET))
                delr_O1_g = float(group_data.get('delr_O1',
                                                 group_data.get('delr_O', 0.0)))
                split_g = float(group_data.get('split_O',
                                               reff_O2_g - reff_O1_g))
                R_O1 = reff_O1_g + delr_O1_g
                R_O2 = reff_O1_g + delr_O1_g + split_g
                err1 = float(group_data.get('delr_O1_err',
                                            group_data.get('delr_O_err', 0.0)))
                err2 = float(np.sqrt(err1 ** 2
                                     + group_data.get('split_O_err', 0.0) ** 2))
                ax2_twin.errorbar(conc, R_O1, yerr=err1, fmt='s', markersize=9,
                                  color=color_O1, markeredgecolor='k',
                                  markeredgewidth=0.8, capsize=4,
                                  alpha=alpha, zorder=5)
                ax2_twin.errorbar(conc, R_O2, yerr=err2, fmt='D', markersize=9,
                                  color=color_O2, markeredgecolor='k',
                                  markeredgewidth=0.8, capsize=4,
                                  alpha=alpha, zorder=5)
            else:
                ...
                R_O = reff_O + delr_O
                ax2_twin.errorbar(conc, R_O, yerr=delr_O_err, fmt='s',
                                  markersize=10,
        ...
        # Add trend lines for bond lengths (per shell when split-mode)
        R_I_means = []
        R_O_means = []
        R_O1_means = []
        R_O2_means = []
        for c in conc_unique:
            groups_at_c = [g for g in data_collection['groups']
                           if g['concentration'] == c]
            if groups_at_c:
                R_I_means.append(np.mean([g['reff_I'] + g.get('delr_I', 0)
                                          for g in groups_at_c]))
                R_O_means.append(np.mean([g['reff_O'] + g.get('delr_O', 0)
                                          for g in groups_at_c]))
                if split_present:
                    r1_vals = [float(g.get('reff_O1', g.get('reff_O', 2.42)))
                               + float(g.get('delr_O1', g.get('delr_O', 0.0)))
                               for g in groups_at_c
                               if 'N_O1' in g and 'N_O2' in g]
                    r2_vals = [float(g.get('reff_O1', g.get('reff_O', 2.42)))
                               + float(g.get('delr_O1', g.get('delr_O', 0.0)))
                               + float(g.get('split_O',
                                             float(g.get('reff_O2',
                                                         g.get('reff_O', 2.42)
                               ...
                               for g in groups_at_c
                               if 'N_O1' in g and 'N_O2' in g]
                    R_O1_means.append(np.mean(r1_vals) if r1_vals else np.nan)
                    R_O2_means.append(np.mean(r2_vals) if r2_vals else np.nan)
        ...
        if split_present:
            R_O1_means = np.array(R_O1_means)[sort_idx]
            R_O2_means = np.array(R_O2_means)[sort_idx]
            ax2_twin.plot(conc_means, R_O1_means, '--', color=color_O1,
                          alpha=0.5, lw=2)
            ax2_twin.plot(conc_means, R_O2_means, '--', color=color_O2,
                          alpha=0.5, lw=2)
        else:
            ax2_twin.plot(conc_means, R_O_means, '--', color=color_O,
                          alpha=0.5, lw=2)

        # Axis labels and formatting
        ax2.set_xlabel('Concentration (M)', fontsize=12, fontweight='bold')
        ax2.set_ylabel('Pb-I Bond Length (\u00c5)', fontsize=12,
                       fontweight='bold', color=color_I)
        ax2.tick_params(axis='y', labelcolor=color_I, labelsize=10)
        ax2.tick_params(axis='x', labelsize=10)
        if split_present:
            ax2_twin.set_ylabel(
                r'Pb-O$_{1}$ / Pb-O$_{2}$ Bond Length (\u00c5)',
                fontsize=12, fontweight='bold', color=color_O1)
            ax2_twin.tick_params(axis='y', labelcolor=color_O1, labelsize=10)
        else:
            ax2_twin.set_ylabel('Pb-O Bond Length (\u00c5)', fontsize=12,
                                fontweight='bold', color=color_O)
    ...
    def _plot_coordination_vs_concentration(self, ax, data):
        ...
        Pb-I in purple. Pb-O is rendered as the split-shell pair (N_O1 short,
        N_O2 long) plus their composite total when the split-shell model is
        active for ANY group; otherwise a single Pb-O series is drawn.
        """
        import numpy as np
        from matplotlib.lines import Line2D
        ...
        # Detect split-shell groups
        split_present = any(('N_O1' in g and 'N_O2' in g) for g in data['groups'])
        ...
        color_O1 = '#DC143C'
        color_O2 = '#FF8C00'
        color_O_total = '#7F1D1D'

        if split_present:
            # Per-group N_O1 / N_O2 / N_O_total scatter.
            for g in data['groups']:
                c = g['concentration']
                if 'N_O1' in g and 'N_O2' in g:
                    ax2.errorbar(c, g['N_O1'], yerr=g.get('N_O1_err', 0.0),
                                 fmt='s', markersize=9,
                                 color=color_O1, markeredgecolor='k', markeredgewidth=0.8,
                                 capsize=4, alpha=0.95, zorder=5)
                    ax2.errorbar(c, g['N_O2'], yerr=g.get('N_O2_err', 0.0),
                                 fmt='D', markersize=9,
                                 color=color_O2, markeredgecolor='k', markeredgewidth=0.8,
                                 capsize=4, alpha=0.95, zorder=5)
                    ax2.errorbar(c, g['N_O1'] + g['N_O2'],
                                 yerr=float(np.sqrt(g.get('N_O1_err', 0.0)**2
                                                    + g.get('N_O2_err', 0.0)**2)),
                                 fmt='^', markersize=10,
                                 color=color_O_total, markeredgecolor='k', markeredgewidth=0.8,
                                 capsize=4, alpha=0.85, zorder=4)
                else:
                    # Group not in split mode: fall back to total N_O.
                    ax2.errorbar(c, g.get('N_O', 0.0), yerr=g.get('N_O_err', 0.0),
                                 fmt='s', markersize=9,
                                 color=color_O, markeredgecolor='k', markeredgewidth=0.8,
                                 capsize=4, alpha=0.6, zorder=4)
            # Trend line on the composite total.
            ax2.plot(conc_sorted, N_O[sort_idx], '--', color=color_O_total, alpha=0.5, lw=2)
            ax2.set_ylabel(r'$N_{Pb-O_1}\,/\,N_{Pb-O_2}\,/\,N_{Pb-O,total}$',
                           fontsize=12, fontweight='bold', color=color_O_total)
            ax2.tick_params(axis='y', labelcolor=color_O_total, labelsize=10)
            ax2.set_ylim(0, 8)
            legend_elements = [
                Line2D([0], [0], marker='o', color='w', markerfacecolor=color_I,
                       markeredgecolor='k', markersize=10, label='$N_{Pb-I}$'),
                Line2D([0], [0], marker='s', color='w', markerfacecolor=color_O1,
                       markeredgecolor='k', markersize=10, label='$N_{Pb-O_1}$ (short)'),
                Line2D([0], [0], marker='D', color='w', markerfacecolor=color_O2,
                       markeredgecolor='k', markersize=10, label='$N_{Pb-O_2}$ (long)'),
                Line2D([0], [0], marker='^', color='w', markerfacecolor=color_O_total,
                       markeredgecolor='k', markersize=10, label='$N_{Pb-O,total}$'),
            ]
        else:
    ...
    def _plot_bondlength_vs_concentration(self, ax, data):
        """
        Pb-I bond length on the left axis. Pb-O is rendered as Pb-O1 (short,
        R = reff_O1 + delr_O1) and Pb-O2 (long, R = reff_O2 + delr_O1 + split_O)
        on the right axis when the split-shell model is active for ANY group.
        Otherwise a single Pb-O series (R = reff_O + delr_O) is drawn.
        """
        import numpy as np
        from matplotlib.lines import Line2D
        ...
        ax.set_xlabel('Concentration (M)', fontsize=12, fontweight='bold')
        ax.set_ylabel('Pb-I Bond Length (\u00c5)', color=color_I,
        ...
        if split_present:
            R_O1_all, R_O2_all, conc_O1, conc_O2 = [], [], [], []
            for g in data['groups']:
                c = g['concentration']
                if 'N_O1' in g and 'N_O2' in g:
                    reff_O1_g = float(g.get('reff_O1', g.get('reff_O', 2.42)))
                    reff_O2_g = float(g.get('reff_O2', reff_O1_g + self.PB_O_SPLIT_TARGET))
                    delr_O1_g = float(g.get('delr_O1', g.get('delr_O', 0.0)))
                    split_g = float(g.get('split_O', reff_O2_g - reff_O1_g))
                    R_O1 = reff_O1_g + delr_O1_g
                    R_O2 = reff_O1_g + delr_O1_g + split_g
                    err1 = float(g.get('delr_O1_err', g.get('delr_O_err', 0.0)))
                    err2 = float(np.sqrt(err1**2 + g.get('split_O_err', 0.0)**2))
                    ax2.errorbar(c, R_O1, yerr=err1, fmt='s', markersize=9,
                                 color=color_O1, markeredgecolor='k',
                                 capsize=4, alpha=0.95, zorder=5)
                    ax2.errorbar(c, R_O2, yerr=err2, fmt='D', markersize=9,
                                 color=color_O2, markeredgecolor='k',
                                 capsize=4, alpha=0.95, zorder=5)
                    R_O1_all.append(R_O1); conc_O1.append(c)
                    R_O2_all.append(R_O2); conc_O2.append(c)
                else:
                    R_O = float(g.get('reff_O', 2.42)) + float(g.get('delr_O', 0.0))
                    ax2.errorbar(c, R_O, yerr=g.get('delr_O_err', 0.0), fmt='s',
                                 color=color_O, markeredgecolor='k',
                                 capsize=4, alpha=0.6, zorder=4)
            # Trend lines per shell.
            if R_O1_all:
                idx1 = np.argsort(conc_O1)
                ax2.plot(np.array(conc_O1)[idx1], np.array(R_O1_all)[idx1],
                         '--', color=color_O1, alpha=0.45, lw=1.5)
            if R_O2_all:
                idx2 = np.argsort(conc_O2)
                ax2.plot(np.array(conc_O2)[idx2], np.array(R_O2_all)[idx2],
                         '--', color=color_O2, alpha=0.45, lw=1.5)
            ax2.set_ylabel(r'Pb-O$_{1}$ / Pb-O$_{2}$ Bond Length (\u00c5)',
                           color=color_O1, fontsize=12, fontweight='bold')
            ax2.tick_params(axis='y', labelcolor=color_O1)
            ax2.set_ylim(2.0, 3.5)
            handles = [
                Line2D([0], [0], marker='o', color='w', markerfacecolor=color_I,
                       markeredgecolor='k', markersize=9, label='Pb-I'),
                Line2D([0], [0], marker='s', color='w', markerfacecolor=color_O1,
                       markeredgecolor='k', markersize=9, label='Pb-O$_{1}$ (short)'),
                Line2D([0], [0], marker='D', color='w', markerfacecolor=color_O2,
                       markeredgecolor='k', markersize=9, label='Pb-O$_{2}$ (long)'),
            ]
            ...
            ax2.set_ylabel('Pb-O Bond Length (\u00c5)', color=color_O,
                           fontsize=12, fontweight='bold')
    ...
    def _resolve_fit_rbkg(self, data_group, default=0.9):
        """Return the best available rbkg-like value for fit window setup."""
        for attr_name in ('optimalBkg_rbkg', 'optimal_rbkg', 'rbkg'):
            value = getattr(data_group, attr_name, None)
            if value is None:
                continue
            try:
                value = float(value)
    ...
    def _resolve_fit_rmin(self, data_group, rbkg, kmin, kmax, kw=3,
                          fallback_rmin=1.67, search_margin=0.15,
                          search_width=2.2):
        """
        Resolve the low-R fit boundary from the first post-rbkg minimum in |chi(R)|.

        The rbkg-associated low-R artifact should not enter the fit window. When a
        stable minimum cannot be identified, fall back to ~1.67 A while still
        staying above rbkg.
        """
        fallback = max(float(fallback_rmin), float(rbkg) + 0.05)
        r = np.array([], dtype=float)
        chir_mag = np.array([], dtype=float)

        try:
            if hasattr(data_group, 'k') and hasattr(data_group, 'chi'):
                temp_group = Group(k=np.asarray(data_group.k), chi=np.asarray(data_group.chi))
                xftf(temp_group, kmin=kmin, kmax=kmax, dk=3,
                     kwindow='hanning', kweight=kw)
                r = np.asarray(getattr(temp_group, 'r', []), dtype=float)
                chir_mag = np.asarray(getattr(temp_group, 'chir_mag', []), dtype=float)
        except Exception:
            r = np.array([], dtype=float)
            chir_mag = np.array([], dtype=float)

        if len(r) != len(chir_mag) or len(r) < 5:
            r = np.asarray(getattr(data_group, 'r', []), dtype=float)
            chir_mag = np.asarray(getattr(data_group, 'chir_mag', []), dtype=float)

        if len(r) != len(chir_mag) or len(r) < 5:
            return fallback, 'fallback'

        finite_mask = np.isfinite(r) & np.isfinite(chir_mag)
        r = r[finite_mask]
        chir_mag = chir_mag[finite_mask]
        if len(r) < 5:
            return fallback, 'fallback'

        search_stop = min(float(np.max(r)), max(float(rbkg) + float(search_width),
                                                fallback + 0.5))
        search_mask = (r >= float(rbkg)) & (r <= search_stop)
        if np.count_nonzero(search_mask) < 5:
            return fallback, 'fallback'

        r_window = r[search_mask]
        y_window = chir_mag[search_mask]

        if len(y_window) >= 7:
            window_length = min(9, len(y_window))
            if window_length % 2 == 0:
                window_length -= 1
            if window_length >= 5:
                y_window = savgol_filter(y_window, window_length=window_length,
                                         polyorder=2, mode='interp')

        maxima = np.where((y_window[1:-1] >= y_window[:-2]) &
                          (y_window[1:-1] >= y_window[2:]))[0] + 1
        minima = np.where((y_window[1:-1] <= y_window[:-2]) &
                          (y_window[1:-1] <= y_window[2:]))[0] + 1
        if len(maxima) == 0 or len(minima) == 0:
            return fallback, 'fallback'

        peak_floor = 0.10 * float(np.max(y_window))
        peak_candidates = [idx for idx in maxima if y_window[idx] >= peak_floor] or list(maxima)
        first_peak = peak_candidates[0]

        min_candidates = [idx for idx in minima
                          if idx > first_peak and r_window[idx] >= float(rbkg) + float(search_margin)]
        if min_candidates:
            detected_rmin = float(r_window[min_candidates[0]])
        else:
            pre_peak_mask = ((r_window >= float(rbkg) + float(search_margin)) &
                             (np.arange(len(r_window)) <= first_peak))
            if np.count_nonzero(pre_peak_mask) == 0:
                return fallback, 'fallback'
            pre_peak_indices = np.where(pre_peak_mask)[0]
            detected_rmin = float(r_window[pre_peak_indices[np.argmin(y_window[pre_peak_mask])]])

        detected_rmin = max(detected_rmin, float(rbkg) + 0.05)
        return detected_rmin, 'detected'
    ...
    def get_transform_for_group(self, data_group):
        ...
        The low-R fit boundary is chosen from the first minimum in |chi(R)| after
        the rbkg-associated artifact. If that minimum cannot be identified
        reliably, a fallback of ~1.67 A is used instead.
        """
        ...
        rbkg = self._resolve_fit_rbkg(data_group, default=0.9)
        # --- FIT RANGES DEFINED HERE ---
        # TransformGroup parameters:
        #   the low-R boundary is taken from the first |chi(R)| minimum after
        #   the rbkg artifact, with a fallback near 1.67 A when needed
        #   kmin, kmax: from group's best_kmin/best_kmax attributes or defaults (3.0, 12.0)
        #   rmin: first |chi(R)| minimum after rbkg (fallback ~1.67 Å)
        #   rmax: fixed at 4.0 Å
        #   dk=3, kweight=kw(default 3), window='hanning'
        rmin, rmin_source = self._resolve_fit_rmin(data_group, rbkg, kmin, kmax, kw=kw)
        trans = TransformGroup(kmin=kmin, kmax=kmax, kweight=kw, dk=3,
                               window='hanning', rmin=rmin, rmax=4.0)
        data_group.fit_rmin = rmin
        data_group.fit_rmin_source = rmin_source
        print(f"  Using rbkg reference = {rbkg:.2f} A")
        print(f"  Using fit rmin = {rmin:.2f} A ({rmin_source})")
    ...
    def enhanced_iteration1_fit(self, feff_base_dir=r'C:/Users/kwill/.larch/feff/relaxed',
        ...
        def process_group(group, group_name):
            ...
            for i, path_info in enumerate(filtered_paths):
                try:
                    ...
                    test_params = self.build_params_for_iteration(
                        1,
                        e0_value=e0,
                        vary_e0=vary_e0,
                    )
            ...
            final_params = param_group(
                ...
                del_e0 = create_param(
                    self._resolve_e0_seed(
                        prev_result=best_path_result['fit_result'],
                        vary_e0=vary_e0,
                        e0_value=e0,
                    ),
                    vary=vary_e0,
        ...
        print("="*70)
    def _get_candidate_paths(self, feff_base_dir, category, apply_default_filter=True, verbose=True):
        """
        Find FEFF paths for a given category.
        
        Parameters:
            feff_base_dir (str or Path): Directory containing FEFF calculation files
            category (str): The category to filter for (e.g., "Pb-O")
            apply_default_filter (bool): Whether to apply the legacy single-scattering
                ``reff < 3`` filter before returning results.
            verbose (bool): Whether to print scan/filter summaries.
        
        Returns:
            list: Candidate path dictionaries.
        """
        import os
        from pathlib import Path
    ...
    def iterative_fit(self, group_identifier, feff_category, iteration, feff_base_dir,
                      vary_e0=False, use_pb_o_aimd_single_shell=False,
                      pb_o_sigma2_mode='restrain',
                      pb_o_sigma2_restraint_strength=1.0):
        """
        ...
        use_pb_o_aimd_for_this_fit = bool(
            use_pb_o_aimd_single_shell and str(feff_category).strip().lower() == 'pb-o'
        )
        if use_pb_o_aimd_for_this_fit:
            pb_o_sigma2_mode = self._validate_pb_o_sigma2_mode(pb_o_sigma2_mode)
        pb_o_penalty_terms = self._build_pb_o_sigma2_penalty_terms(
            sigma2_param_name='pbo_sig2',
            enabled=use_pb_o_aimd_for_this_fit,
            pb_o_sigma2_mode=pb_o_sigma2_mode,
            pb_o_sigma2_restraint_strength=pb_o_sigma2_restraint_strength,
        )
        ...
        try:
            ...
            del_e0_value = self._resolve_e0_seed(prev_fit, vary_e0=vary_e0)
            ...
            try:
                ...
                if feff_category.lower() == 'pb-o':
                    self._configure_pb_o_single_shell_path(
                        ...
                        'pbo_sig2',
                        'pbo_delr',
                        use_pb_o_aimd_single_shell=use_pb_o_aimd_for_this_fit,
                    )
                else:
                ...
                def create_fixed_params():
                    params = param_group(
                        ...
                        del_e0 = create_param(del_e0_value, vary=vary_e0, min=-15, max=15),
                    ...
                    if use_pb_o_aimd_for_this_fit:
                        ...
                        params.pbo_sig2 = create_param(0.01, vary=True, min=0.0, max=0.5)
                    return params
                test_params = create_fixed_params()
                test_fit = self._feffit_with_optional_penalties(
                    test_params,
                    dset,
                    penalty_terms=pb_o_penalty_terms,
                )
                ...
                if use_pb_o_aimd_for_this_fit and 'c3_o' in test_fit.params:
                    test_results[-1]['c3_o'] = test_fit.params['c3_o'].value
        ...
        def create_final_params():
            ...
            if use_pb_o_aimd_for_this_fit:
                self._apply_pb_o_aimd_sigma2_constraint(
                    params,
                    sigma2_param_name='pbo_sig2',
                    sigma2_value=best_candidate['pbo_sig2'],
                    third_param_name='c3_o',
                    c3_value=best_candidate.get('c3_o', 0.0),
                    pb_o_sigma2_mode=pb_o_sigma2_mode,
                    pb_o_sigma2_restraint_strength=pb_o_sigma2_restraint_strength,
                    sigma2_min=0.0,
                    sigma2_max=0.08,
                ...
                params.pbo_sig2 = create_param(best_candidate['pbo_sig2'], vary=True, min=0.0, max=0.5)
            return params
        final_params = create_final_params()
        final_dset = feffit_dataset(data=temp_data, pathlist=final_paths, transform=trans)
        final_fit = self._feffit_with_optional_penalties(
            final_params,
            final_dset,
            penalty_terms=pb_o_penalty_terms,
        )
        ...
        if 'c3_o' in final_fit.params:
            print(f"  c3_o = {final_fit.params['c3_o'].value:.5f}")
        ...
        return True
    # def iterative_fit_all(self, feff_category, iteration, feff_base_dir, previous_params=None, physical_sanity_mode='off'):
    #     """
    ...
    def iterative_fit_all(self, feff_category, iteration, feff_base_dir, previous_params=None,
                          physical_sanity_mode='off', vary_e0=False,
                          use_pb_o_aimd_single_shell=False,
                          pb_o_sigma2_mode='restrain',
                          pb_o_sigma2_restraint_strength=1.0,
                          aicc_improvement_threshold=2.0,
                          pb_o_aicc_improvement_threshold=0.0,
                          pb_o_model='single'):
        """
        Perform an iterative EXAFS fit for all groups by evaluating candidate FEFF paths
        and adding the best ones to the fit model if they improve the R-factor.
        ...
        previous_params (dict, optional): Dictionary of previous iteration parameters
                                            keyed by group_id.
        aicc_improvement_threshold (float, optional): Minimum ΔAICc required to
                                            accept a new path in the general case.
        pb_o_aicc_improvement_threshold (float, optional): Relaxed minimum ΔAICc
                                            required for Pb-O path addition.
                                            Defaults to 0.0.
        pb_o_model (str, optional): Pb-O fit model. One of:
            - 'single' (default): single Pb-O shell, current behavior
            - 'single_c3': single shell with third cumulant (use together with
              ``use_pb_o_aimd_single_shell=True``)
            - 'split_two_shell': two Pb-O paths (short + long), independent N,
              fixed sigma^2=0.025 A^2, and a constrained 0.30-0.38 A radial split.
              Only applies when ``feff_category=='Pb-O'``.
        ...
        feff_category_key = str(feff_category).strip().lower()
        active_aicc_threshold = float(
            pb_o_aicc_improvement_threshold if feff_category_key == 'pb-o'
            else aicc_improvement_threshold
        )
        print(f"AICc improvement threshold for {feff_category}: {active_aicc_threshold:.3f}")
        if use_pb_o_aimd_for_this_fit:
            pb_o_sigma2_mode = self._validate_pb_o_sigma2_mode(pb_o_sigma2_mode)
            print(
                "Pb-O AIMD single-shell enabled: "
                f"mode={pb_o_sigma2_mode}, sigma2 target=0.055225 A^2, "
                f"restraint strength={pb_o_sigma2_restraint_strength}"
            )
        pb_o_penalty_terms = self._build_pb_o_sigma2_penalty_terms(
            sigma2_param_name='pbo_sig2',
        ...
        summary = []
        feff_base_dir = Path(feff_base_dir)
        all_candidate_infos = self._get_candidate_paths(
            feff_base_dir,
            feff_category,
        ...
        if all_candidate_infos:
            for i, info in enumerate(all_candidate_infos[:5]):
                label = info.get('label', info.get('filename', 'unknown'))
                reff = info.get('reff', np.nan)
                print(f"   {i+1:2d}: {label}, reff = {reff:.3f} Å")
            if len(all_candidate_infos) > 5:
                print(f"      ... ({len(all_candidate_infos)-5} more paths)")
        else:
            print(f"No candidate '{feff_category}' FEFF paths were found in {feff_base_dir}.")
            return return_params

        # ----------------------------------------------------------------
        # Pb-O split two-shell branch (alternative to single-shell candidate loop)
        # ----------------------------------------------------------------
        pb_o_model_norm = str(pb_o_model or 'single').lower()
        if pb_o_model_norm not in ('single', 'single_c3', 'split_two_shell'):
            raise ValueError(
                f"pb_o_model must be 'single', 'single_c3', or 'split_two_shell'; "
                f"got {pb_o_model!r}"
            )
        if pb_o_model_norm == 'single_c3' and not use_pb_o_aimd_single_shell:
            print("  NOTE: pb_o_model='single_c3' is equivalent to 'single' "
                  "unless use_pb_o_aimd_single_shell=True is also passed.")

        if pb_o_model_norm == 'split_two_shell' and feff_category_key == 'pb-o':
            from larch.xafs import feffit, feffit_dataset, feffpath
            from larch.fitting import param, param_group
            from larch import Group as _LGroup

            print(f"\n{'='*70}")
            print("Pb-O SPLIT TWO-SHELL MODEL")
            print(f"{'='*70}")
            print(f"  sigma^2 fixed at {self.PB_O_SPLIT_SIGMA2:.4f} A^2 (AIMD-informed)")
            print(f"  split_O constrained to "
                  f"[{self.PB_O_SPLIT_MIN:.2f}, {self.PB_O_SPLIT_MAX:.2f}] A "
                  f"(target {self.PB_O_SPLIT_TARGET:.2f} A)")
            print(f"  N_O1, N_O2 in [0, {self.PB_O_SPLIT_N_MAX:.0f}]; "
                  f"N_total checked against {self.PB_O_SPLIT_NTOTAL_MAX:.0f}")
            print(f"  AICc gate: ΔAICc <= {active_aicc_threshold:.3f}")
            print(f"  No third cumulant.")

            info_O1, info_O2, reff_O1, reff_O2, sel_diag = self._select_split_pb_o_paths(
                all_candidate_infos
            )
            print(f"  Path selection: {sel_diag.get('reason', '')}")
            if info_O1 is None:
                print("  Split-shell model NOT applicable; no groups fit. "
                      "Falling back to caller for alternative handling.")
                return return_params

            for proj_name, project in self.projects.items():
                print(f"\nProcessing project (split): {proj_name}")
                for gname, group in project.groups.items():
                    group_id = f"{proj_name}.{gname}"
                    print(f"\n  Group: {group_id}")
                    if not (hasattr(group, 'k') and hasattr(group, 'chi')):
                        print(f"    Skipping (missing k/chi).")
                        continue
                    prev_fit = getattr(group, f"fit_result_iter{iteration-1}", None)
                    prev_paths = getattr(group, f"paths_iter{iteration-1}", None) or []
                    if prev_fit is None:
                        print(f"    Skipping (no fit_result_iter{iteration-1}).")
                        continue
                    ...
                    try:
                        ...
                        print(f"    Error reading prev params: {exc}")
                        continue

                    # Carry-forward record (used if split fit is rejected).
                    return_params[group_id] = {
                        'amp': amp_value,
                        'del_e0': del_e0_value,
                        'n_PbI': n_PbI_value,
                    ...
                    # Clamp n_PbI seed into the [1.5, 6.0] window so the
                    # solver does not start outside the new bounds.
                    n_PbI_seed = float(np.clip(n_PbI_value, 1.5, 6.0))
                    params = param_group(
                        amp=param(amp_value, vary=False, min=0.0, max=1.5),
                        del_e0=param(del_e0_value, vary=vary_e0, min=-15.0, max=15.0),
                        n_PbI=param(n_PbI_seed, vary=True, min=1.5, max=6.0),
                        pbi_sig2=param(pbi_sig2_value, vary=True, min=0.0, max=0.05),
                        pbi_delr=param(pbi_delr_value, vary=True, min=-0.10, max=0.10),
                    )
                    self._add_split_pb_o_parameters(
                        params,
                        reff_O1=reff_O1, reff_O2=reff_O2,
                    ...
                    # Re-attach prior Pb-I paths (deepcopy preserves their expressions).
                    new_paths = copy.deepcopy(prev_paths)

                    # Build the two split Pb-O paths.
                    pbo1 = feffpath(info_O1['feffpath'].filename)
                    self._configure_split_pb_o_path(
                        pbo1, role='O1',
                        reff_O1=reff_O1, reff_O2=reff_O2,
                        naming='flat', e0_name='del_e0',
                        label_prefix=info_O1.get('label', 'Pb-O'),
                    )
                    pbo2 = feffpath(info_O2['feffpath'].filename)
                    self._configure_split_pb_o_path(
                        pbo2, role='O2',
                        reff_O1=reff_O1, reff_O2=reff_O2,
                        naming='flat', e0_name='del_e0',
                        label_prefix=info_O2.get('label', 'Pb-O'),
                    )
                    new_paths.extend([pbo1, pbo2])

                    trans = self.get_transform_for_group(group)
                    trans.rmax = trans.rmax + 0.2 * iteration
                    temp_data = _LGroup(k=group.k, chi=group.chi)
                    dset = feffit_dataset(data=temp_data, pathlist=new_paths, transform=trans)

                    try:
                        final_fit = self._feffit_with_optional_penalties(
                            params, [dset], penalty_terms=None,
                        )
                        success = final_fit is not None and hasattr(final_fit, 'rfactor')
                    except Exception as exc:
                        print(f"    Split fit raised: {exc}")
                        continue
                    if not success or final_fit is None:
                        print(f"    Split fit did not converge.")
                        continue

                    prev_aicc = self._compute_aicc(prev_fit).get('aicc', float('nan'))
                    final_aicc = self._compute_aicc(final_fit).get('aicc', float('nan'))
                    delta_aicc = (final_aicc - prev_aicc) if (
                        np.isfinite(prev_aicc) and np.isfinite(final_aicc)
                    ) else float('nan')

                    summary = self._evaluate_split_pb_o_summary(
                        final_fit, reff_O1=reff_O1, reff_O2=reff_O2, naming='flat'
                    )

                    aicc_accept = (np.isfinite(delta_aicc)
                                   and delta_aicc <= active_aicc_threshold)
                    sanity_required = physical_sanity_mode in ('all', 'final')
                    sanity_ok = summary['passed'] or not sanity_required
                    # Iteration 2 with split_two_shell is a SEEDING step:
                    # always accept the two Pb-O paths so iter 3+ inherits them.
                    accepted = True
                    seed_note = (f" [seed; AICc gate bypassed | ΔAICc={delta_aicc:+.2f}, "
                                 f"sanity={'OK' if summary['passed'] else 'WARN'}]")

                    self._print_split_pb_o_summary(
                        summary, group_id=group_id, accepted=accepted,
                        delta_aicc=delta_aicc, rfactor=final_fit.rfactor,
                    )
                    print(f"    Seeding mode:{seed_note}")
                    if not aicc_accept:
                        print(f"    (Note: ΔAICc={delta_aicc:+.2f} > {active_aicc_threshold:.2f}, "
                              f"but split-shell paths are seeded for iter {iteration+1}+ refinement.)")

                    if accepted:
                        setattr(group, f'fit_result_iter{iteration}', final_fit)
                        setattr(group, f'paths_iter{iteration}', new_paths)
                        setattr(group, f'pb_o_split_meta_iter{iteration}', {
                            'reff_O1': reff_O1, 'reff_O2': reff_O2,
                            'info_O1': info_O1.get('label'),
                            'info_O2': info_O2.get('label'),
                            'summary': summary,
                            'delta_aicc': delta_aicc,
                        ...
                        return_params[group_id].update({
                            'rfactor': final_fit.rfactor,
                            'paths': [info_O1.get('label'), info_O2.get('label')],
                            'iteration': iteration,
                            'pb_o_model': 'split_two_shell',
                            'n_PbO1': summary['n_O1'],
                            'n_PbO2': summary['n_O2'],
                            'n_PbO_total': summary['n_O_total'],
                            'pbo_split': summary['split_O'],
                            'reff_O1': reff_O1,
                            'reff_O2': reff_O2,
                        })
                    else:
                        # Carry forward previous fit so later iterations still have data.
                        setattr(group, f'fit_result_iter{iteration}', prev_fit)
                        setattr(group, f'paths_iter{iteration}', prev_paths)
                        reason_bits = []
                        if not aicc_accept:
                            reason_bits.append(f"ΔAICc={delta_aicc:+.2f} > {active_aicc_threshold:.2f}")
                        if sanity_required and not summary['passed']:
                            reason_bits.append(f"sanity warnings: {len(summary['warnings'])}")
                        print(f"    Rejected ({'; '.join(reason_bits) or 'unknown'}); "
                              f"carrying forward iter {iteration-1}.")

            return return_params
        # ----------------------------------------------------------------
        # End Pb-O split branch
        # ----------------------------------------------------------------

        def get_filtered_candidate_pool(rmin_fit, rmax_fit):
            """Return a cached filtered candidate list for the current fit window."""
            reff_tolerance = 0.02
            max_candidates = 50
            ss_only = iteration <= 2
            ...
            cached = candidate_pool_cache.get(filter_key)
            if cached is not None:
                return list(cached['paths']), cached['summary'], True

            if feff_category.upper() == 'PB-O':
                reff_min = 2.1
                reff_max = 2.9
                filtered = [p for p in all_candidate_infos if reff_min <= p.get('reff', 0) <= reff_max]
                filter_label = f"Pb-O reff constraint: {reff_min:.1f} <= reff <= {reff_max:.1f} Å"
            else:
                filtered = [p for p in all_candidate_infos if rmin_fit <= p.get('reff', 0) <= rmax_fit]
                filter_label = f"R-range filter: {rmin_fit:.2f} <= reff <= {rmax_fit:.2f} Å"
            ...
            filtered = sorted(
                filtered,
                key=lambda x: (0 if x.get('path_type', '').lower() == 'ss' else 1,
                               x.get('reff', np.inf))
            )
            ...
            if len(filtered) > max_candidates:
                ...
                limit_summary = (
                    f"Limited to {max_candidates} candidates: "
                    f"{len(selected_ss)} SS + {len(selected_ms)} MS paths"
                )

            summary_info = {
                'total_candidates': len(all_candidate_infos),
                'filter_label': filter_label,
                'before_ss_filter': before_ss_filter,
                'before_dedup': before_dedup,
                'final_count': len(filtered),
                'ss_only': ss_only,
                'reff_tolerance': reff_tolerance,
                'limit_summary': limit_summary,
            }
            candidate_pool_cache[filter_key] = {
                'paths': list(filtered),
                'summary': summary_info,
            }
            return list(filtered), summary_info, True
            ...
            for gname, group in project.groups.items():
                ...
                try:
                    ...
                    if previous_params and group_id in previous_params:
                        group_params = previous_params[group_id]
                        print(f"  Using provided parameters for {group_id}")
                        del_e0_value = group_params.get(
                            'del_e0',
                            self._resolve_e0_seed(prev_fit, vary_e0=vary_e0)
                        )
                        n_PbI_value = group_params.get('n_PbI', prev_fit.params['n_PbI'].value)
                        pbi_sig2_value = group_params.get('pbi_sig2', prev_fit.params['pbi_sig2'].value)
                        pbi_delr_value = group_params.get('pbi_delr', prev_fit.params['pbi_delr'].value)
                ...
                # --- Collect candidate FEFF paths for the desired category ---
                print(f"  Using pre-scanned candidate '{feff_category}' paths...")
                candidate_infos = list(all_candidate_infos)
                ...
                #   R-range: rmin=first |chi(R)| minimum after rbkg
                #            (fallback ~1.67 Å), rmax=4.0 Å
                #            (extended by +0.2*iteration below)
                trans = self.get_transform_for_group(group)
                trans.rmax = trans.rmax + 0.2 * iteration
                rmin_fit = trans.rmin + 0.3
                rmax_fit = trans.rmax
                filtered, filter_summary, used_cache = get_filtered_candidate_pool(rmin_fit, rmax_fit)
                print(f"    Using cached candidate pool: {filter_summary['final_count']} paths (from {filter_summary['total_candidates']} total).")
                ...
                if not used_cache and feff_category.upper() == 'PB-O':
                    pbo_reff_min = 2.1
                    pbo_reff_max = 2.9
                ...
                if not used_cache and iteration <= 2:
                    before_ss_filter = len(filtered)
                    filtered = [p for p in filtered if p.get('path_type', '').lower() == 'ss']
                ...
                for i, info in enumerate(filtered):
                    ...
                    try:
                        ...
                        def create_fixed_params():
                            ...
                            params = param_group(**params_dict)
                            if use_pb_o_aimd_for_this_fit:
                                self._apply_pb_o_aimd_sigma2_constraint(
                                    params,
                                    sigma2_param_name='pbo_sig2',
                                ...
                                setattr(
                                    params,
                                    param_names['sig2'],
                                    create_param(0.01, vary=True, min=0.0, max=0.03)
                                )
                            return params
                ...
                for cand in test_results:
                    ...
                    cand['sane'], cand['sanity_reason'] = self._fit_is_physically_sane(cand['fit_result'], paths=[cand['path']])

                # Choose candidate pool based on physical_sanity_mode
                ...
                candidate_pool.sort(
                    key=lambda x: (
                        x['aicc'],
                        0 if x.get('sane', False) else 1,
                        x.get('rfactor', np.inf),
                        x.get('delr_abs', np.inf),
                    )
                )
                ...
                try:
                    ...
                    def create_final_params():
                        ...
                        if use_pb_o_aimd_for_this_fit:
                            self._apply_pb_o_aimd_sigma2_constraint(
                                params,
                                sigma2_param_name='pbo_sig2',
                                sigma2_value=best_candidate[final_param_names['sig2']],
                                third_param_name='c3_o',
                                c3_value=best_candidate.get('c3_o', 0.0),
                                pb_o_sigma2_mode=pb_o_sigma2_mode,
                                pb_o_sigma2_restraint_strength=pb_o_sigma2_restraint_strength,
                            ...
                            setattr(
                                params,
                                final_param_names['sig2'],
                                create_param(best_candidate[final_param_names['sig2']], vary=True, min=0.0, max=0.03)
                            )
                        return params
                    final_params = create_final_params()
                    final_dset = feffit_dataset(data=temp_data, pathlist=final_paths, transform=trans)
                    ...
                    # Only accept if AICc improved by the active threshold.
                    if delta_aicc < active_aicc_threshold:
                        print(f"    Path rejected: ΔAICc={delta_aicc:.3f} < threshold ({active_aicc_threshold:.3f}). Keeping previous model.")
                        continue
                    ...
                    if 'c3_o' in final_fit.params:
                        return_params[group_id]['c3_o'] = final_fit.params['c3_o'].value
                    ...
                    if 'c3_o' in final_fit.params:
                        fit_info += (
                            f"\nc3_o = {final_fit.params['c3_o'].value:.5f} "
                            f"Â± {final_fit.params['c3_o'].stderr or 0:.5f}"
                        )
                    ax_r.text(0.05, 0.95, fit_info, transform=ax_r.transAxes, fontsize=8,
                            verticalalignment='top',
                            bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
...
# ============================
# Processed-data checkpoint helpers
# ============================

def _xasmu2r_is_picklable(value):
    try:
        pickle.dumps(value)
        return True
    except Exception:
...
def _xasmu2r_snapshot_group(group):
    """Create a picklable snapshot of a Larch group by keeping only picklable attributes."""
    attrs = {}
    skipped = []
    for key, val in getattr(group, '__dict__', {}).items():
        if key.startswith('__'):
            continue
        if _xasmu2r_is_picklable(val):
            attrs[key] = val
        else:
            skipped.append(key)
    return {'attrs': attrs, 'skipped': skipped}


def _xasmu2r_restore_group(snapshot):
    grp = Group()
    for key, val in snapshot.get('attrs', {}).items():
        try:
            setattr(grp, key, val)
        except Exception:
            pass
...
def _xasmu2r_save_processed_checkpoint(self, output_dir=None, filename='processed_checkpoint.pkl', metadata_filename='processed_checkpoint_meta.json', verbose=True):
    """
    Save all currently loaded project/group data into a checkpoint that can be loaded later
    for fitting without rerunning Athena import and FT processing.

    Parameters
    ----------
    output_dir : str or Path or None
        Destination folder. Defaults to ``self.prj_folder / 'processed'``.
    filename : str
        Pickle snapshot filename.
    ...
    -------
    dict
        Paths and summary information for the saved checkpoint.
    """
    base_dir = Path(output_dir) if output_dir is not None else (Path(self.prj_folder) / 'processed')
    base_dir.mkdir(parents=True, exist_ok=True)

    snapshot = {
        'source_prj_folder': str(self.prj_folder),
        'projects': {},
    }
    meta = {
        'source_prj_folder': str(self.prj_folder),
        'projects': {},
        'checkpoint_file': str(base_dir / filename),
    }

    total_groups = 0
    total_skipped = 0
    for proj_name, project in getattr(self, 'projects', {}).items():
        proj_groups = {}
        proj_meta = {}
        groups = getattr(project, 'groups', {}) or {}
        for group_name, group in groups.items():
            snap = _xasmu2r_snapshot_group(group)
            proj_groups[group_name] = snap
            proj_meta[group_name] = {
                'n_saved_attrs': len(snap.get('attrs', {})),
                'skipped_attrs': snap.get('skipped', []),
            }
            total_groups += 1
            total_skipped += len(snap.get('skipped', []))
        snapshot['projects'][proj_name] = {'groups': proj_groups}
        meta['projects'][proj_name] = proj_meta

    checkpoint_path = base_dir / filename
    meta_path = base_dir / metadata_filename
    ...
    if verbose:
        print("\n" + "="*80)
        print("SAVE PROCESSED CHECKPOINT SUMMARY")
        print("="*80)
        print(f"  Source folder:     {self.prj_folder}")
        print(f"  Output folder:     {base_dir}")
        print(f"  Projects saved:    {len(snapshot['projects'])}")
        print(f"  Groups saved:      {total_groups}")
        print(f"  Skipped attrs:     {total_skipped}")
    ...
    return {
        ...
        'projects_saved': len(snapshot['projects']),
        'groups_saved': total_groups,
        'skipped_attrs': total_skipped,
    }


def _xasmu2r_load_processed_checkpoint(self, processed_dir=None, filename='processed_checkpoint.pkl', verbose=True):
    """Load a processed-data checkpoint into ``self.projects``."""
    base_dir = Path(processed_dir) if processed_dir is not None else (Path(self.prj_folder) / 'processed')
    checkpoint_path = base_dir / filename
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"Processed checkpoint not found: {checkpoint_path}")
    ...
    projects = {}
    total_groups = 0
    for proj_name, proj_snap in snapshot.get('projects', {}).items():
        project = SimpleNamespace()
        project.groups = {}
        for group_name, group_snap in proj_snap.get('groups', {}).items():
            project.groups[group_name] = _xasmu2r_restore_group(group_snap)
            total_groups += 1
        projects[proj_name] = project

    self.projects = projects
    self.prj_folder = base_dir
    self.processed_checkpoint_path = checkpoint_path

    if verbose:
        print("\n" + "="*80)
        print("LOAD PROCESSED CHECKPOINT SUMMARY")
        print("="*80)
        print(f"  Processed folder:  {base_dir}")
        print(f"  Checkpoint file:   {checkpoint_path}")
        print(f"  Projects loaded:   {len(projects)}")
        print(f"  Groups loaded:     {total_groups}")
        print("="*80)

    return {
        'processed_dir': str(base_dir),
        'checkpoint_file': str(checkpoint_path),
        'projects_loaded': len(projects),
        'groups_loaded': total_groups,
    }


def _xasmu2r_save_processed_after_ft(self, output_dir=None, filename='processed_checkpoint.pkl', metadata_filename='processed_checkpoint_meta.json', verbose=True):
    """Convenience wrapper for saving a checkpoint after FT processing has been completed."""
    return _xasmu2r_save_processed_checkpoint(self, output_dir=output_dir, filename=filename, metadata_filename=metadata_filename, verbose=verbose)


XASmu2r.save_processed_checkpoint = _xasmu2r_save_processed_checkpoint
XASmu2r.load_processed_checkpoint = _xasmu2r_load_processed_checkpoint
XASmu2r.save_processed_after_ft = _xasmu2r_save_processed_after_ft
