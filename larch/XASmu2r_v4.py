...
class XASmu2r:
    ...
    def iterative_fit_linked_all(self, feff_category, iteration, feff_base_dir, 
                                ...
                                physical_sanity_mode='off', require_valid_aicc=False,
                                allow_rfactor_fallback=True, reject_on_bound_pinning=False,
                                force_add_path=False, force_add_min_paths=0):
        """
        ...
        for proj_name, project in self.projects.items():
            ...
            for group_name, group in project.groups.items():
                ...
                for cand in filtered[:max_paths_to_add * 2]:
                    ...
                    try:
                        ...
                        if not sane:
                            ...
                            if physical_sanity_mode == 'all' and not force_add_path:
                                test_results.append(cand_record)
                                continue
                        ...
                        if not cand_aicc['valid'] and require_valid_aicc and not force_add_path:
                            cand_record['rejection_reason'] = f'AICc invalid: {cand_aicc["reason"]}'
                            test_results.append(cand_record)
                            continue

                        delta = cand_aicc['aicc'] - prev_aicc_info['aicc'] if cand_aicc['valid'] and prev_aicc_info['valid'] else np.inf
                        cand_record['delta_aicc'] = delta
                        improvement = prev_rfactor - result.rfactor
                        cand_record['improvement'] = improvement
                ...
                # Rank candidates for normal acceptance and for optional force-add fallback
                ranked_candidates = [tr for tr in test_results if tr.get('result') is not None]
                sane_candidates = [tr for tr in ranked_candidates if tr['sanity_pass'] and np.isfinite(tr['aicc'])]
                sane_candidates.sort(key=lambda x: x['aicc'])

                # Best candidate anyway: prefer valid AICc, else lowest R-factor
                best_candidate_anyway = None
                valid_aicc_candidates = [tr for tr in ranked_candidates if np.isfinite(tr.get('aicc', np.inf))]
                if valid_aicc_candidates:
                    valid_aicc_candidates.sort(key=lambda x: x['aicc'])
                    best_candidate_anyway = valid_aicc_candidates[0]
                elif ranked_candidates and allow_rfactor_fallback:
                    ranked_candidates.sort(key=lambda x: x['rfactor'])
                    best_candidate_anyway = ranked_candidates[0]
                ...
                forced_add_used = False
                if not paths_to_add and force_add_path and best_candidate_anyway is not None and force_add_min_paths > 0:
                    paths_to_add = [best_candidate_anyway]
                    forced_add_used = True
                    print(f"    FORCE ADD: storing best {feff_category} candidate despite failed acceptance gates")
                    print(f"    Forced path: {best_candidate_anyway['path_info']['label']}")
                ...
                if not forced_add_used:
                    if physical_sanity_mode == 'all' and not final_sane:
                    ...
                    if final_aicc['valid'] and prev_aicc_info['valid']:
                        if (prev_aicc_info['aicc'] - final_aicc['aicc']) < aicc_improvement_threshold:
                            if not (allow_rfactor_fallback and (prev_rfactor - final_result.rfactor) > 0):
                    ...
                    if not final_sane:
                        print(f"    FORCE ADD WARNING: final fit failed sanity ({final_reason})")
                    if not final_aicc['valid']:
                        print(f"    FORCE ADD WARNING: final AICc invalid ({final_aicc.get('reason','unknown')})")
        ...
        for group_id, res in results.items():
            if 'rfactor' in res:
                ...
                improved = "F" if res.get('forced_add') else ("✓" if res.get('improved') else "")
        ...
        for pname in params:
            if 'sigma2' in pname.lower():
                ...
                if self._parameter_is_near_bound(p, frac=0.01):
                    ...
                    if getattr(self, '_sanity_reject_on_bound_pinning', True):
                        if verbose:
                            print(f'    Sanity FAIL: {reason}')
                        return False, reason
                    elif verbose:
    ...
    def replace_large_delr_paths(
        ...
        old_reject_on_bound_pinning = getattr(self, '_sanity_reject_on_bound_pinning', True)
        self._sanity_reject_on_bound_pinning = reject_on_bound_pinning
        ...
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                ...
                for idx, old_path, role, delr_val in flagged:
                    ...
                    for cand in nearby:
                        try:
                            ...
                            # Physical sanity check (configurable scope)
                            sane = True
                            sane_reason = 'OK'
                            target_only_paths = [trial_paths[idx]]
                            if sanity_scope == 'global':
                                sane, sane_reason = self._fit_is_physically_sane(
                                    refit_result, paths=trial_paths, verbose=False)
                            elif sanity_scope == 'target_only':
                                sane, sane_reason = self._fit_is_physically_sane(
                                    refit_result, paths=target_only_paths, verbose=False,
                                    coordination_floors={})
                            elif sanity_scope == 'off':
                                sane, sane_reason = True, 'sanity disabled'
                            else:
                                sane, sane_reason = True, f'unknown sanity_scope={sanity_scope}; treated as off'
                            ...
                            aicc_valid = cand_aicc['valid'] and pre_repl_aicc['valid']
                            if require_valid_aicc and not aicc_valid:
                                continue

                            aicc_improved = aicc_valid and ((pre_repl_aicc['aicc'] - cand_aicc['aicc']) >= aicc_improvement_threshold)
                            rfactor_improved = (best_group_fit.rfactor - refit_result.rfactor) >= improvement_tolerance

                            accept_metric = aicc_improved or (allow_rfactor_fallback and rfactor_improved)
                            if not accept_metric:
                                continue

                            # Score: prefer lower AICc when valid, otherwise lower R-factor, then smaller |ΔR|
                            score = (cand_aicc['aicc'] if cand_aicc['valid'] else np.inf, refit_result.rfactor, abs(new_delr), abs(cand['reff'] - target_reff))
                ...
                if accepted_replacements:
                    # Final AICc/sanity summary for the overall replacement
                    ...
                    if sanity_scope == 'global':
                        ...
                        target_paths = [p for p in best_group_paths if self._infer_path_role(p) in target_param_types] if target_param_types else best_group_paths
                        final_sane, final_reason = self._fit_is_physically_sane(
                            best_group_fit, paths=target_paths, verbose=verbose, coordination_floors={})
                    else:
                        final_sane, final_reason = True, 'sanity disabled'
        ...
        print("=" * 80 + "\n")
        self._sanity_reject_on_bound_pinning = old_reject_on_bound_pinning
        return results
    ...
    def iterative_fit_all(self, feff_category, iteration, feff_base_dir, previous_params=None,
        ...
        physical_sanity_mode : str, optional
            ...
            Default is 'off', which is appropriate for bottom-up model building.
