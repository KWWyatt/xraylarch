...
class XASmu2r:
    ...
    def ft_processing_batch(self, kmax_range=(10.0, 14.0), kmin_zc_range=(1.0, 3.0),
        ...
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                ...
                print(f"    Candidates tested: {num_kmin_cand} kmin × {num_kmax_cand} kmax = {num_kmin_cand * num_kmax_cand} combinations")
    ...
    def iterative_fit_linked_all(self, feff_category, iteration, feff_base_dir,
                                ...
                                physical_sanity_mode='off', force_add_path=False,
                                force_add_min_paths=0, require_valid_aicc=False,
        ...
        This method is intended for later-stage model growth, typically after Pb-I and Pb-O
        shells have already been incorporated. Candidate paths are evaluated with AICc when
        available, with optional fallback to R-factor ranking. Physical sanity can be enforced,
        used diagnostically, or bypassed depending on the workflow stage.

        Parameters
        ----------
        feff_category : str
            Category of paths to add, e.g. 'Pb-S' or 'MS'.
        iteration : int
            Current iteration number (typically >= 3).
        feff_base_dir : str or Path
            Directory containing FEFF calculations.
        include_ms : bool
            If True, include multiple-scattering candidates.
        max_paths_to_add : int
            Maximum number of new paths to add per group.
        verbose : bool
            Print detailed progress.
        plot_results : bool
            Plot before/after fits when a model is accepted or force-added.
        aicc_improvement_threshold : float
            Minimum AICc improvement required for normal acceptance.
        physical_sanity_mode : {'off', 'final', 'all'}
            'off'   : never reject in this method on physical sanity; print warnings only.
            'final' : ignore sanity during candidate scan, warn on final fit.
            'all'   : enforce sanity during candidate scan and final acceptance.
        force_add_path : bool
            If True, force-add the best available candidate path when no candidate passes the
            normal acceptance gates. Useful when one path of a category must exist in every model.
        force_add_min_paths : int
            Minimum number of paths that must be added when force_add_path is True.
        require_valid_aicc : bool
            If True, reject candidates lacking valid AICc. If False, allow fallback to R-factor.
        allow_rfactor_fallback : bool
            If True, use R-factor ranking/acceptance when AICc is unavailable.
        reject_on_bound_pinning : bool
            If True, bound-pinned sanity failures remain hard rejects even in permissive modes.

        Returns
        -------
        dict
            Results for each processed group.
        """
        import os
        from pathlib import Path
        import copy
        ...
        def is_hard_bound_failure(reason):
            if not reason:
                return False
            r = str(reason).lower()
            if 'pinned at bound' in r or 'near bound' in r:
                return True
            return False

        def candidate_rank_key(record):
            # Prefer valid AICc, else fallback to R-factor, else by small |ΔR| if available.
            aicc = record.get('aicc', np.inf)
            rfac = record.get('rfactor', np.inf)
            path_obj = record.get('path')
            delr_mag = np.inf
            if path_obj is not None and record.get('result') is not None:
                val = self._evaluate_path_delr(path_obj, record['result'])
                if val is not None:
                    delr_mag = abs(val)
            return (0 if np.isfinite(aicc) else 1, aicc, rfac, delr_mag)
        ...
        for proj_name, project in self.projects.items():
            ...
            for group_name, group in project.groups.items():
                ...
                trans = self.get_transform_for_group(group)
                filtered = [p for p in candidate_paths if trans.rmin <= p.get('reff', 0) <= trans.rmax + 0.5]

                if feff_category.upper() == 'MS':
                    existing_ss_elements = set()
                    for path in prev_paths:
                        if hasattr(path, '_feffdat') and hasattr(path._feffdat, 'geom'):
                            if path._feffdat.nleg == 2:
                        ...
                        for cand in filtered:
                            if cand.get('path_type') == 'ms':
                                ...
                                if scatterers.issubset(existing_ss_elements):
                                    ...
                                    if verbose:
                                        print(f"    Skipping {cand.get('label', 'MS path')}: missing SS for {missing}")
                            else:
                                ms_filtered.append(cand)
                ...
                if not filtered:
                    ...
                    results[group_id] = {
                        ...
                        'improved': False,
                        'reason': 'No candidates in R-range',
                        'paths_added': 0,
                    }
                    continue
                ...
                # Evaluate candidates. Scan a slightly wider pool than max_paths_to_add.
                test_results = []
                scan_limit = min(len(filtered), max(max_paths_to_add * 3, force_add_min_paths * 4, 8))
                for cand in filtered[:scan_limit]:
                    cand_record = {
                    ...
                    try:
                        ...
                        sane, reason = self._fit_is_physically_sane(
                            result, paths=test_paths, verbose=False)
                        cand_record['sanity_pass'] = sane

                        if not sane and verbose:
                            print(f"      {cand['label'][:35]:35s} SANITY FAIL: {reason}")

                        # Only treat sanity as hard rejection when explicitly requested.
                        if not sane and physical_sanity_mode == 'all':
                            if reject_on_bound_pinning or not is_hard_bound_failure(reason):
                        ...
                        if not cand_aicc['valid'] and require_valid_aicc:
                            cand_record['rejection_reason'] = f'AICc invalid: {cand_aicc["reason"]}'
                            test_results.append(cand_record)
                            continue

                        if prev_aicc_info['valid'] and cand_aicc['valid']:
                            delta = cand_aicc['aicc'] - prev_aicc_info['aicc']
                            cand_record['delta_aicc'] = delta
                        else:
                            cand_record['delta_aicc'] = np.nan

                        cand_record['force_eligible'] = True
                        improvement = prev_rfactor - result.rfactor
                        if verbose:
                            print(f"      {cand['label'][:35]:35s} "
                                  f"AICc={cand_record['aicc']:+.2f}  "
                                  f"ΔAICc={cand_record['delta_aicc']:+.2f}  "
                                  f"Rfactor ΔR={improvement:+.6f}")
                    except Exception as e:
                        cand_record['rejection_reason'] = f'exception: {e}'
                    test_results.append(cand_record)

                # Normal acceptance pool
                eligible = []
                for tr in test_results:
                    if tr['result'] is None:
                        continue
                    if require_valid_aicc and not np.isfinite(tr['aicc']):
                        continue
                    if physical_sanity_mode == 'all' and not tr['sanity_pass']:
                        continue
                    eligible.append(tr)

                eligible.sort(key=candidate_rank_key)

                paths_to_add = []
                for sc in eligible:
                    accepted = False
                    if prev_aicc_info['valid'] and np.isfinite(sc['aicc']):
                        if (prev_aicc_info['aicc'] - sc['aicc']) >= aicc_improvement_threshold:
                            accepted = True
                    elif allow_rfactor_fallback:
                        if (prev_rfactor - sc['rfactor']) >= 1e-6:
                            accepted = True
                    elif not require_valid_aicc:
                        accepted = True
                ...
                force_added = False
                force_added_records = []
                if (not paths_to_add) and force_add_path and force_add_min_paths > 0:
                    force_candidates = [tr for tr in test_results if tr.get('force_eligible') and tr.get('result') is not None]
                    force_candidates.sort(key=candidate_rank_key)
                    force_added_records = force_candidates[:force_add_min_paths]
                    if force_added_records:
                        paths_to_add = force_added_records
                        force_added = True
                        print(f"    FORCE ADD enabled: storing best available {feff_category} candidate(s) despite normal rejection.")

                if not paths_to_add:
                    best_reason = 'no candidates passed selection criteria'
                    if eligible:
                        best = eligible[0]
                        best_reason = f"best ΔAICc={best.get('delta_aicc', np.nan):+.2f}, threshold={aicc_improvement_threshold}"
                    elif test_results:
                        reasons = [tr['rejection_reason'] for tr in test_results if tr['rejection_reason']]
                        if reasons:
                            best_reason = '; '.join(list(dict.fromkeys(reasons))[:3])
                    ...
                    results[group_id] = {
                        ...
                        'paths_added': 0,
                        'forced': False,
                        'aicc_old': prev_aicc_info.get('aicc', np.nan),
                        'aicc_new': np.nan,
                        'delta_aicc': np.nan,
                    }
                    continue
                ...
                for pa in paths_to_add:
                    ...
                    tag = "FORCE-ADDED" if force_added else "accepted"
                    print(f"    + {pa['path_info']['label']}  ({tag}; ΔAICc={pa.get('delta_aicc', np.nan):+.2f})")
                ...
                if not success or final_result is None:
                    ...
                    results[group_id] = {
                        ...
                        'improved': False,
                        'converged': False,
                        'reason': 'final fit did not converge',
                        'paths_added': len(paths_to_add),
                        'forced': force_added,
                        'aicc_old': prev_aicc_info.get('aicc', np.nan),
                        'aicc_new': np.nan,
                        'delta_aicc': np.nan,
                ...
                final_sane, final_reason = self._fit_is_physically_sane(
                    final_result, paths=final_paths, verbose=False)
                final_aicc = self._compute_aicc(final_result)
                metrics = self._evaluate_model_selection_metrics(prev_fit, final_result)

                if not final_sane:
                    if physical_sanity_mode == 'all' and not force_added:
                        print(f"    REJECTED final fit: sanity failed ({final_reason})")
                        results[group_id] = {
                            'rfactor': prev_rfactor, 'improved': False,
                            'reason': f'final sanity: {final_reason}',
                            'paths_added': 0, 'forced': False,
                            'aicc_old': prev_aicc_info.get('aicc', np.nan),
                            'aicc_new': np.nan, 'delta_aicc': np.nan,
                        }
                        continue
                    else:
                        print(f"    WARNING: Final fit sanity issue: {final_reason}")

                if (not force_added) and final_aicc['valid'] and prev_aicc_info['valid']:
                    if (prev_aicc_info['aicc'] - final_aicc['aicc']) < aicc_improvement_threshold:
                        if not allow_rfactor_fallback or (prev_rfactor - final_result.rfactor) <= 1e-6:
                            ...
                            results[group_id] = {
                                ...
                                'paths_added': 0, 'forced': False,
                                'aicc_old': prev_aicc_info.get('aicc', np.nan),
                                'aicc_new': final_aicc.get('aicc', np.nan),
                                'delta_aicc': metrics.get('delta_aicc', np.nan),
                            }
                            continue
                ...
                status_label = "FORCE-ADDED" if force_added else "ACCEPTED"
                print(f"\n    {status_label}: R-factor {prev_rfactor:.6f} -> {final_result.rfactor:.6f}")
                print(f"    AICc {prev_aicc_info['aicc']:.2f} -> {final_aicc['aicc']:.2f}  "
                      f"(ΔAICc={metrics['delta_aicc']:+.2f})")

                results[group_id] = {
                    'rfactor': final_result.rfactor,
                    ...
                    'forced': force_added,
                    'converged': True,
                    'path_labels_added': [pa['path_info']['label'] for pa in paths_to_add],
                }
                total_processed += 1
                if improvement > 0:
                    total_improved += 1
        ...
        print(f"  Groups force-added: {total_force_added}")
        print("-"*80)
        print("  Results per group:")
        print(f"  {'Group':<45s}  {'R_prev':>10s}  {'R_new':>10s}  {'AICc_old':>10s}  {'AICc_new':>10s}  {'ΔAICc':>8s}  {'#p':>3s}  {'Status':>12s}")
        print("-"*80)
        for group_id, res in results.items():
            r_new = res.get('rfactor', np.nan)
            improvement = res.get('improvement', 0.0)
            r_prev = r_new + improvement if np.isfinite(r_new) else np.nan
            paths_added = res.get('paths_added', 0)
            aicc_old = res.get('aicc_old', np.nan)
            aicc_new = res.get('aicc_new', np.nan)
            d_aicc = res.get('delta_aicc', np.nan)
            if res.get('forced'):
                status = 'FORCE-ADDED'
            elif res.get('improved'):
                status = 'ACCEPTED'
            else:
                status = res.get('reason', 'SKIPPED')[:12]
            print(f"    {group_id:<40s}  {r_prev:>10.6f}  {r_new:>10.6f}  "
                  f"{aicc_old:>10.2f}  {aicc_new:>10.2f}  {d_aicc:>+8.2f}  "
                  f"{paths_added:>3d}  {status:>12s}")
        print("="*80 + "\n")
        return results
        ...
        for proj_name, project in self.projects.items():
            ...
            for gname, group in project.groups.items():
                ...
                for cand in test_results:
                    ...
                    cand['sane'], cand['sanity_reason'] = self._fit_is_physically_sane(cand['fit_result'])

                # Filter to only sane candidates
                sane_candidates = [c for c in test_results if c['sane'] and np.isfinite(c['aicc'])]
                if not sane_candidates:
                    print(f"    No physically sane candidates with valid AICc for {group_id}. Keeping previous model.")
                    continue

                # Sort by AICc
                sane_candidates.sort(key=lambda x: x['aicc'])
                best_candidate = sane_candidates[0]
                print(f"    Best candidate for {group_id}: {best_candidate['label']} ({best_candidate.get('path_type', 'unknown')})")
                print(f"    AICc range: {min(c['aicc'] for c in sane_candidates):.3f} - {max(c['aicc'] for c in sane_candidates):.3f}")
                ...
                try:
                    ...
                    # Physical sanity check
                    is_sane, sanity_reason = self._fit_is_physically_sane(final_fit)
                    if not is_sane:
    ...
    def collect_fit_summary(self, iteration, prefer_shared_e0=False, only_fitted=True):
        """
        Collect a compact summary of stored fit results for a given iteration.

        Parameters
        ----------
        iteration : int
            Iteration number to summarize.
        prefer_shared_e0 : bool
            If True and a shared-e0 fit exists for the iteration, prefer it over the
            standard fit result.
        only_fitted : bool
            If True, omit groups without stored fit results.
        ...
        for proj_name, project in self.projects.items():
            for group_name, group in project.groups.items():
                group_id = f"{proj_name}.{group_name}"
                fit = None
                fit_source = f"fit_result_iter{iteration}"
                if prefer_shared_e0:
                    fit = getattr(group, f"fit_result_iter{iteration}_shared_e0", None)
                    if fit is not None:
                        fit_source = f"fit_result_iter{iteration}_shared_e0"
                if fit is None:
                    fit = getattr(group, f"fit_result_iter{iteration}", None)
                    fit_source = f"fit_result_iter{iteration}"

                paths = getattr(group, f"paths_iter{iteration}", [])
                if fit is None and only_fitted:
                    continue

                params = getattr(fit, 'params', None) if fit is not None else None
                aicc = self._compute_aicc(fit)['aicc'] if fit is not None else None
                path_labels = [getattr(p, 'label', 'unknown') for p in (paths or [])]

                rec = {
                    'group_id': group_id,
                    'project': proj_name,
                    'group_name': group_name,
                    'fit_source': fit_source if fit is not None else None,
                    'rfactor': getattr(fit, 'rfactor', None) if fit is not None else None,
                    'chi_square': getattr(fit, 'chi_square', None) if fit is not None else None,
                    'chi2': getattr(fit, 'chi2', None) if fit is not None else None,
                    'redchi': getattr(fit, 'redchi', None) if fit is not None else None,
                    'aicc': aicc,
                    'n_paths': len(paths or []),
                    'paths': " | ".join(path_labels),
                    'e0': _safe_param(params, 'e0', 'del_e0'),
                    'N_I': _safe_param(params, 'N_I', 'n_PbI'),
                    'N_O': _safe_param(params, 'N_O', 'n_PbO'),
                    'N_S': _safe_param(params, 'N_S', 'n_PbS'),
                    'sigma2_I': _safe_param(params, 'sigma2_I', 'pbi_sig2'),
                    'sigma2_O': _safe_param(params, 'sigma2_O', 'pbo_sig2'),
                    'sigma2_S': _safe_param(params, 'sigma2_S', 'pbs_sig2'),
                    'delr_I': _safe_param(params, 'delr_I', 'pbi_delr'),
                    'delr_O': _safe_param(params, 'delr_O', 'pbo_delr'),
                    'delr_S': _safe_param(params, 'delr_S', 'pbs_delr'),
                    'has_fit': fit is not None,
                }
                records.append(rec)
        return records

    def print_fit_diagnostics(self, iteration, prefer_shared_e0=False,
                              rfactor_warn=0.08, delr_warn=0.10):
        """
        Print a compact diagnostic summary for decision making after a fitting stage.
        """
        import math
        records = self.collect_fit_summary(iteration, prefer_shared_e0=prefer_shared_e0, only_fitted=True)
        print("\n" + "="*88)
        print(f"FIT DIAGNOSTICS SUMMARY (Iteration {iteration})")
        print("="*88)
        print(f"{'Group':<44s} {'R':>8s} {'AICc':>10s} {'N_I':>7s} {'N_O':>7s} {'N_S':>7s} {'dR_I':>8s} {'dR_O':>8s} {'dR_S':>8s}")
        print("-"*88)
        for rec in records:
            def f(v, fmt):
                try:
                    ...
                    return 'nan'
            print(f"{rec['group_id']:<44s} {f(rec['rfactor'], '.4f'):>8s} {f(rec['aicc'], '.2f'):>10s} "
                  f"{f(rec['N_I'], '.2f'):>7s} {f(rec['N_O'], '.2f'):>7s} {f(rec['N_S'], '.2f'):>7s} "
                  f"{f(rec['delr_I'], '+.3f'):>8s} {f(rec['delr_O'], '+.3f'):>8s} {f(rec['delr_S'], '+.3f'):>8s}")
        print("-"*88)

        high_r = [r for r in records if r.get('rfactor') is not None and r['rfactor'] > rfactor_warn]
        high_delr = [r for r in records if any(
            (r.get(k) is not None and abs(r[k]) > delr_warn) for k in ('delr_I', 'delr_O', 'delr_S')
        )]
        low_no = [r for r in records if r.get('N_O') is not None and r['N_O'] < 1.0]
        print(f"Groups with R-factor > {rfactor_warn:.3f}: {len(high_r)}")
        if high_r:
            print("  " + ", ".join(r['group_id'] for r in high_r[:10]))
        print(f"Groups with |ΔR| > {delr_warn:.3f} Å on I/O/S: {len(high_delr)}")
        if high_delr:
            print("  " + ", ".join(r['group_id'] for r in high_delr[:10]))
        print(f"Groups with N_O < 1.0: {len(low_no)}")
        if low_no:
            print("  " + ", ".join(r['group_id'] for r in low_no[:10]))
        print("="*88 + "\n")
        return records

    def export_fit_summary(self, iteration, output_dir='fit_exports',
                           prefer_shared_e0=False, base_name=None,
                           make_publication_figure=True, figure_kwargs=None,
                           print_diagnostics=True):
        """
        Export fit summary tables and an optional publication figure for a chosen iteration.

        Files written
        -------------
        - <base_name>.csv
        - <base_name>.json
        - <base_name>_figure.png  (optional)
        ...
        if base_name is None:
            base_name = f"iteration_{iteration}_fit_summary"
        ...
        if print_diagnostics:
            self.print_fit_diagnostics(iteration, prefer_shared_e0=prefer_shared_e0)

        csv_path = output_dir / f"{base_name}.csv"
        json_path = output_dir / f"{base_name}.json"
        fig_path = output_dir / f"{base_name}_figure.png"

        fieldnames = [
            'group_id', 'project', 'group_name', 'fit_source', 'rfactor', 'chi_square',
            'chi2', 'redchi', 'aicc', 'n_paths', 'paths',
            'e0', 'N_I', 'N_O', 'N_S', 'sigma2_I', 'sigma2_O', 'sigma2_S',
            'delr_I', 'delr_O', 'delr_S', 'has_fit'
        ]
        ...
        made_figure = False
        if make_publication_figure:
            try:
                kwargs = dict(figure_kwargs or {})
                kwargs.setdefault('save_path', str(fig_path))
                self.plot_publication_figure(**kwargs)
                made_figure = True
                print(f"Saved publication figure to {fig_path}")
            except Exception as e:
                print(f"WARNING: Could not generate publication figure: {e}")

        print(f"Saved fit summary CSV to {csv_path}")
        print(f"Saved fit summary JSON to {json_path}")

        return {
            'csv': str(csv_path),
            'json': str(json_path),
            'figure': str(fig_path) if made_figure else None,
        }

    def finalize_publishable_results(self, iteration, output_dir='fit_exports',
                                     prefer_shared_e0=False, make_publication_figure=True,
                                     figure_kwargs=None):
        """
        Convenience wrapper to export the current best-fit state after manual model refinement.
        """
        print("\n" + "="*88)
        print("FINALIZING PUBLISHABLE RESULTS")
        print("="*88)
        print(f"Iteration selected for export: {iteration}")
        exported = self.export_fit_summary(
            iteration=iteration,
            output_dir=output_dir,
            prefer_shared_e0=prefer_shared_e0,
            make_publication_figure=make_publication_figure,
            figure_kwargs=figure_kwargs,
            print_diagnostics=True,
        )
        print("Recommended next check before publication:")
        print("  1. Inspect groups with high R-factor or large |ΔR| from the diagnostics block.")
        print("  2. Confirm the path list in the CSV matches the chemistry you want to report.")
        print("  3. Use the exported CSV/JSON as the source for manuscript tables and SI tables.")
        print("="*88 + "\n")
        return exported
